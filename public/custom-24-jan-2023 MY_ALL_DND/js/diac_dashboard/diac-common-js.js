...
$(document).ready(function () {
	...
	// on every 3 minutes check for the notification
...
if ($(".multiple_miscellaneous_initial_preview").val()) {
	var files = JSON.parse($(".multiple_miscellaneous_initial_preview").val());
	console.log(files);
	fileOptions.initialPreview = [];
	fileOptions.initialPreviewAsData = true;
	fileOptions.overwriteInitial = true;
	fileOptions.initialPreviewConfig = [{ type: "pdf", width: "100%", key: 1 }];
	fileOptions.initialPreviewShowDelete = true;

	if (files) {
		for (var i = 0; i < files.length; i++) {
			console.log(files[i]);
			fileOptions.initialPreview.push(
...
$(".documents-input").fileinput(fileOptions);
