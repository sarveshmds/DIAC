...
function viewFDForm(event) {
	...
	var diac_txn_id = oTable.fnGetData(row)["diac_txn_id"];
	...
	table += "<tr><th>DIAC Txn. ID: </th> <td>" + diac_txn_id + "</td></tr>";
...
function btnEditFDForm(event) {
	...
	var fd_diac_txn_id = oTable.fnGetData(row)["diac_txn_id"];
	...
	$("#fd_diac_txn_id").val(fd_diac_txn_id);
...
var dataTable = $("#dataTableFeeReleasedList").DataTable({
	...
	columns: [
		...
		},
		{
			...
			render: function (data, type, row, meta) {
				return (
					'<input type="hidden" name="released_to_code" id="released_to_code" value="' +
					data.released_to +
					'">'
				);
			},
...
function viewFRForm(event) {
	...
	var released_to = oTable.fnGetData(row)["arbitrator_name"];
