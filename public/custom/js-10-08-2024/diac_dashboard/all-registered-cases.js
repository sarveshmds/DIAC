...
var dataTableGeneralCases = $("#dataTableCaseform").DataTable({
	...
	columns: [
		...
		{ data: "registered_on" },
		{
			...
			render: function (data, type, row, meta) {
				return (
					'<span class="badge btn-info">' + data.case_status_dec + "</span>"
				);
			},
		},
		{
...
// ================================================================================
// On click excel generate button
$(document).on("click", "#btn_rc_excel_download", function () {
	var case_no = $("#ac_f_case_no").val();
	var case_title = $("#ac_f_case_title").val();
	var arbitration_petition = $("#ac_f_arb_pet").val();
	var toa = $("#ac_f_toa").val();
	var registered_on_from = $("#ac_f_registered_on_from").val();
	var registered_on_to = $("#ac_f_registered_on_to").val();
	var date_of_award_from = $("#ac_f_date_of_award_from").val();
	var date_of_award_to = $("#ac_f_date_of_award_to").val();
	var reffered_by = $("#ac_f_reffered_by").val();
	var reffered_by_judge = $("#ac_f_reffered_by_judge").val();
	var referred_on = $("#ac_f_referred_on").val();
	var name_of_arbitrator = $("#ac_f_name_of_arbitrator").val();
	var case_status = $("#ac_f_status").val();
	var sorting_by = $("#sorting_by").val();
	var sort_to = $("#sort_to").val();
	var case_type = "GENERAL";

	var excel_url =
		base_url +
		"excel/registered-cases-list" +
		"?case_no=" +
		encodeURIComponent(case_no) +
		"&case_title=" +
		encodeURIComponent(case_title) +
		"&arbitration_petition=" +
		encodeURIComponent(arbitration_petition) +
		"&toa=" +
		encodeURIComponent(toa) +
		"&registered_on_from=" +
		encodeURIComponent(registered_on_from) +
		"&registered_on_to=" +
		encodeURIComponent(registered_on_to) +
		"&date_of_award_from=" +
		encodeURIComponent(date_of_award_from) +
		"&date_of_award_to=" +
		encodeURIComponent(date_of_award_to) +
		"&reffered_by=" +
		encodeURIComponent(reffered_by) +
		"&reffered_by_judge=" +
		encodeURIComponent(reffered_by_judge) +
		"&referred_on=" +
		encodeURIComponent(referred_on) +
		"&name_of_arbitrator=" +
		encodeURIComponent(name_of_arbitrator) +
		"&case_status=" +
		encodeURIComponent(case_status) +
		"&sorting_by=" +
		encodeURIComponent(sorting_by) +
		"&sort_to=" +
		encodeURIComponent(sort_to) +
		"&case_type=" +
		encodeURIComponent(case_type);

	// Redirect or open the generated URL to initiate the download
