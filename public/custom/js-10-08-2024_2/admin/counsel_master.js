...
var counsel_setup_datatable = $("#counsel_setup_datatable_appr").DataTable({
	...
	ajax: {
		...
		data: function (d) {
			...
			d.f_counsel_name = $("#f_counsel_name").val();
		},
	},
	columns: [
		{
		...
		},
		{
			...
			sClass: "alignCenter",
			render: function (data, type, row, meta) {
				return `${data.perm_address_1 ? data.perm_address_1 : ""} ${
					data.perm_address_2 ? "<br />" + data.perm_address_2 : ""
				} ${data.perm_country_name ? "<br />" + data.perm_country_name : ""} ${
					data.perm_state_name ? "<br />" + data.perm_state_name : ""
				} ${data.perm_pincode ? "<br />" + data.perm_pincode : ""}`;
			},
		},
		{
			data: null,
			...
			sClass: "alignCenter",
			render: function (data, type, row, meta) {
				return `${data.corr_address_1 ? data.corr_address_1 : ""} ${
					data.corr_address_2 ? "<br />" + data.corr_address_2 : ""
				} ${data.corr_country_name ? "<br />" + data.corr_country_name : ""} ${
					data.corr_state_name ? "<br />" + data.corr_state_name : ""
				} ${data.corr_pincode ? "<br />" + data.corr_pincode : ""}`;

				// return "<span>" + data.corr_address_1
				// 	?  data.corr_address_1
				// 	: "" +
				// 	  (data.corr_address_2 ? "<br /> hello" + data.corr_address_2 : "") +
				// 	  (data.corr_country_name ? "<br />" + data.corr_country_name : "") +
				// 	  "<br>" +
				// 	  data.corr_state_name
				// 	? "<br />" + data.corr_state_name
				// 	: "" + data.corr_pincode
				// 	? "<br />" + data.corr_pincode
				// 	: "" + "</span>";
			},
		},
		{
			...
			render: function (data, type, row, meta) {
				return (
					...
					"counsel-setup/edit/" +
					data.code +
					'" class="btn btn-warning btn-sm" data-tooltip="tooltip" title="Edit Details"><span class="fa fa-edit"></span></a> <button class="btn btn-danger btn-sm" onclick="delete_counsel_master(' +
					data.code +
...
$("#f_cl_btn_submit").on("click", function () {
	counsel_setup_datatable.ajax.reload();
});
