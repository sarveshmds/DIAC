	...
	columns: [
		...
		{ data: "remarks" },
		{
			...
			render: function (data, type, row, meta) {
				...
				$button = "";
				if (data.active_status != 3) {
					$button += `<a href="${base_url}cause-list/re-schedule?case_no=${
						data.slug
					}&id=${btoa(
						data.id
					)}" class="btn btn-primary btn-sm"  data-tooltip="tooltip" title="Re-schedule Hearing" target="_BLANK"><span class="fa fa-calendar"></span></a> `;
				}
...
// ===========================================================
// Re-schedule hearing form
// Add cause list form
$("#form_reschedule_cause_list").bootstrapValidator({
	message: "This value is not valid",
	submitButtons: 'button[type="submit"]',
	submitHandler: function (validator, form, submitButton) {
		$("#cause_list_btn_submit").attr("disabled", "disabled");
		var formData = new FormData(
			document.getElementById("form_reschedule_cause_list")
		);
		urls = base_url + "service/cause_list_operation";
		$.ajax({
			url: urls,
			...
			success: function (response) {
				...
				if (obj.status == true) {
					...
					swal(
						...
						function () {
							window.location.href = `${base_url}cause-list`;
						}
					);
