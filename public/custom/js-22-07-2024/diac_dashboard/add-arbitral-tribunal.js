...
$(document).on("change", "#at_whe_on_panel", function () {
	...
	$(".is_emp_col").hide();
	$(".not_emp_col").hide();
	reset_forms_values_only();

	if (at_whe_on_panel == 1) {
		$(".is_emp_col").show();
	}
	if (at_whe_on_panel == 2) {
		$(".not_emp_col").show();
	}
});
	...
	$.ajax({
		...
		success: function (response) {
			...
			if (response.status == true) {
				...
				$("#at_new_arb_name").attr("readonly", "readonly");
...
// Get data by select not empanelled arbitrator
$(document).on("change", "#not_emp_arb_name", function () {
	var arb_code = $(this).val();
...
function reset_forms_values_only() {
	$("#at_new_arb_name").val("");
	$("#at_new_arb_name").attr("readonly", "readonly");

	$("#at_arb_name").val(null).trigger("change");
	$("#not_emp_arb_name").val(null).trigger("change");
	...
	// ===============================
	// Permanent address
	$("#permanent_address_1").val("").attr("readonly", false);
	$("#permanent_address_2").val("").attr("readonly", false);
	$("#permanent_country option").prop("selected", false);
	...
	$("#permanent_pincode").val("").attr("readonly", false);

	// ===============================
	// Corresponding address
	$("#arb_tri_corr_address_1").val("").attr("readonly", false);
	$("#arb_tri_corr_address_2").val("").attr("readonly", false);
	$("#arb_tri_corr_country option").prop("selected", false);
	$("#arb_tri_corr_country ").attr("disabled", false);
	$("#arb_tri_corr_state").html("<option value=''>Select State</option>");
	$("#arb_tri_corr_state ").attr("disabled", false);
	$("#arb_tri_corr_pincode").val("").attr("readonly", false);
}

function reset_arb_tri_form() {
...
function btnEditATForm(event) {
	...
	if (whether_on_panel == 1) {
		...
		$(".not_emp_col").show();
		$("#not_emp_arb_name").val(arbitrator_code).trigger("change");
		fetch_arbitrator_details(arbitrator_code);
...
// ------------------------------------------------------
$("#btn_send_arb_tri_notification").on("click", function () {
	let case_no = $(this).data("case-no");
	if (case_no) {
		swal(
			{
				type: "info",
				title: "Are you sure?",
				text: "You want to send the notification to everyone.",
				showCancelButton: true,
				confirmButtonColor: "#67e60a",
			...
			function (isConfirm) {
				if (isConfirm) {
					$.ajax({
						url:
							base_url + "service/arbitral_tribunal_notification_to_everyone",
						type: "POST",
						data: {
							case_no: case_no,
							csrf_trans_token: $("#csrf_at_trans_token").val(),
