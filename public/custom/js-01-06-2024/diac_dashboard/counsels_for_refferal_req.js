...
// ================================================
// Datatable initialization for NEW CASE REGISTRATION
// ================================================
var dataTableCounselListForNR = $("#dataTableCounselListForNR").DataTable({
	processing: true,
	serverSide: true,
	autoWidth: false,
	responsive: true,
	...
	columns: [
		...
		{ data: "phone_number" },
		{
			...
			render: function (data, type, row, meta) {
				...
				$button +=
					'<button class="btn btn-warning btn-sm" onclick="btnEditCounselList(event)" data-tooltip="tooltip" title="Edit"><span class="fa fa-edit"></span></button> ';
...
$("#addCounselListModal").on("hidden.bs.modal", function (e) {
	...
	$("#counsel_op_type").val("ADD_CASE_COUNSEL_FOR_REF_REQ");
...
function viewCounselForm(event) {
	...
	var oTable = $("#dataTableCounselListForNR").dataTable();
	var row;
...
function btnEditCounselList(event) {
	// Get data table instance to get the data through row
	var oTable = $("#dataTableCounselListForNR").dataTable();
	var row;
	if (event.target.tagName == "BUTTON")
	...
	var code = oTable.fnGetData(row)["code"];
	// var name = oTable.fnGetData(row)["name"];
	var name_code = oTable.fnGetData(row)["name_code"];
	var appearing_for = oTable.fnGetData(row)["appearing_for"];
	// var enrollment_no = oTable.fnGetData(row)["enrollment_no"];
	// var cl_res_name = oTable.fnGetData(row)["cl_res_name"];
	// var cl_res_count_number = oTable.fnGetData(row)["cl_res_count_number"];
	// var email = oTable.fnGetData(row)["email"];
	...
	console.log(appearing_for);
	$("#hidden_counsel_id").val(id);
	$("#hidden_counsel_code").val(code);
	$("#counsel_op_type").val("UPDATE_CASE_COUNSEL_FOR_REF_REQ");

	$('#counsels_list option[value="' + name_code + '"]').prop("selected", true);
	$("#counsels_list").select2().trigger("change");

	$('#counsel_appearing_for option[value="' + appearing_for + '"]').prop(
		"selected",
		true
	);

	$(".counsel-modal-title").html(
		'<span class="fa fa-edit"></span> Update Counsel Details'
	);
	$("#addCounselListModal").modal({
		backdrop: "static",
		keyboard: false,
...
$("#counsel_form").bootstrapValidator({
	...
	submitHandler: function (validator, form, submitButton) {
		...
		var counsels_list_option = $("#counsels_list").val();
				...
				if (obj.status == true) {
					swal(
						...
						function () {
							...
							dataTableCounselListForNR.ajax.reload();
							if (counsels_list_option == "OTHER") {
								fetch_counsels_list();
							}
						}
					);
...
$(document).on("change", "#counsels_list", function () {
	...
	if (counsel_code != "OTHER" && counsel_code != "" && counsel_code != null) {
		...
		// $("#counsel_appearing_for option").prop("selected", false);
...
/**
 * On click counsel nav load the claimant and respondents data
 */

$("#btn_counsel_nav_tab").on("click", function () {
	$.ajax({
		url: base_url + "service/get_all_claimant_respondent_list",
		method: "POST",
		data: {
			case_no: $("#hidden_refferal_req_case_code").val(),
		},
		success: function (response) {
			// Enable the submit button
			...
			try {
				...
				if (obj.status == true) {
					...
					lists.forEach((list) => {
						options += `<option value="${list.code}">${list.name} (${list.count_number}) ${list.type}</option>`;
					});
					$("#counsel_appearing_for").html(options);
				} else if (obj.status === "validationerror") {
					swal({
						title: "Validation Error",
						text: obj.msg,
...
/**
 * Fetch the counsels list again
 */

function fetch_counsels_list() {
	$.ajax({
		url: base_url + "service/get_all_counsels_list_for_dropdown",
		method: "POST",
		data: {},
		success: function (response) {
			try {
				...
				if (obj.status == true) {
					...
					let options =
						'<option value="">Select</option><option value="OTHER">Other</option>';
					lists.forEach((list) => {
						options += `<option value="${list.code}">${list.name} ${
							list.enrollment_no ? " - (" + list.enrollment_no + ")" : ""
						}</option>`;
					});
					$("#counsels_list").html(options);
					$("#counsels_list").select2();
				} else {
					swal({
						title: "Error",
						text: "Server error, please refresh your browser to load the data correctly",
						type: "error",
						html: true,
					});
				}
				...
				sweetAlert(
					"Sorry",
					"Server error, please refresh your browser to load the data correctly",
					"error"
				);
			}
		},
