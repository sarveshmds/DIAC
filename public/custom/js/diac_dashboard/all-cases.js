...
var dataTableCaseRegistered = $("#dataTableCaseRegistered").DataTable({
	processing: true,
	serverSide: true,
	autoWidth: false,
	responsive: true,
...
$("#ac_f_btn").on("click", function () {
	dataTableCaseRegistered.ajax.reload();
});
			...
			function (isConfirm) {
				if (isConfirm) {
					$.ajax({
						...
						data: {
							id: id,
							csrf_trans_token: $("#csrf_crl_trans_token").val(),
						},
						success: function (response) {
							try {
								...
								if (obj.status == false) {
									...
									dataTableCaseRegistered = $(
										"#dataTableCaseRegistered"
									).DataTable();
									dataTableCaseRegistered.draw();
									dataTableCaseRegistered.clear();
								}
							} catch (e) {
