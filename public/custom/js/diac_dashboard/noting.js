		...
		},
		{
			...
			render: function (data, type, row, meta) {
				...
				if (role_code == "DIAC") {
					...
					buttons += `<button class="btn btn-danger btn-sm" data-tooltip="tooltip" title="Delete" onclick="btnDeleteNotingList('${data.id}', '${data.case_no}')"><span class="fa fa-trash"></span></button>`;
				}

				// Show the mark done button as per the roles
				if (
				...
				// Show the mark done status to coordinator
				if (role_code == "HEAD_COORDINATOR") {
					buttons += "";
					if (data.task_done == 0) {
						buttons += " <span class='badge bg-blue'>Pending</span>";
					} else {
						buttons += " <span class='badge bg-green'>Done</span>";
					}
				}
...
function btnDeleteNotingList(id, case_no) {
	if (id) {
		swal(
			{
				type: "error",
