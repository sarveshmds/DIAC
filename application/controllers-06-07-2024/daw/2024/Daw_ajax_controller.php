...
class Daw_ajax_controller extends CI_Controller
{
	...
	*/
    ...
    {
        echo json_encode($this->daw_2024_model->get_daw_all_live_sessions());
    }
                ...
                if ($this->form_validation->run()) {
                    ...
                    try {
                        ...
                        if ($hidden_ls_code && $this->input->post('op_type') == 'EDIT_LIVE_SESSION') {
                            $result = $this->db->where('code', $hidden_ls_code)->update('daw_live_sessions_details_tbl_2024', $ls_data);
                            $dbmessage = "Live session updated successfully";
                        } else {
                            // Insert case details
                            $result = $this->db->insert('daw_live_sessions_details_tbl_2024', $ls_data);
    ...
    public function registration_approval()
    {
        ...
        $result = $this->db->where('id', $this->input->post('hidden_id'))->update('daw_registrations_tbl_2024', $data);

        if ($result) {

            // Fetch person details
            $data['person'] = $this->daw_2024_model->get_single_registration_data($this->input->post('hidden_id'));
            ...
            if ($application_status == 1) {
                ...
                // Send mail
                $sendStatus = send_daw_registration_approval_mail_2024($data['person'], DAW_REG_MAIL_PDF_STORAGE_PATH . $filename);
            ...
            if ($application_status == 2) {
                ...
                // Send mail
                $sendStatus = send_daw_registration_rejection_mail_2024($data['person']);
            ...
            if ($application_status == 3) {
                ...
                // Send mail
                $sendStatus = send_daw_payment_pending_mail_2024($data['person']);
            ...
            if ($application_status == 4) {
                ...
                // Send mail
                $sendStatus = send_daw_payment_received_mail_2024($data['person']);
