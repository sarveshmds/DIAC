                    ...
                    if ($this->form_validation->run()) {
                        ...
                        try {
                            ...
                            if ($result) {
                                ...
                                // Check if the generated case number is already registered by some other person.
                                // This can be scenario when multiple user hit final submit at the same time
                                $check_case_number = $this->db->where([
                                    'case_no' => $case_no,
                                    'status' => 1
                                ])->from('cs_case_details_tbl')->count_all_results();

                                if ($check_case_number > 0) {
                                    failure_response('Exception Error: The same case number is generated again. So we are aborting the case registration process, please try again. ', false, true);
                                }
                                ...
                                $additional_phones = [];
                                array_push($additional_phones, $deputy_counsel['phone_number']);
                                array_push($additional_phones, $case_manager['phone_number']);
                                array_push($additional_phones, $additional_coordinator['phone_number']);
                                ...
                                if (count($all_phone_numbers) > 0) {
                                    foreach ($all_phone_numbers as $key => $phone_number) {
                                        ...
                                        $whatsapp_result = wa_case_registration_info($phone_number, $full_case_no, $case_details_data, $deputy_counsel);
                                ...
                                $subject = 'Registration of fresh case with DIAC :-' . $full_case_no . ' (' . $case_details_data['case_title'] . ')';
