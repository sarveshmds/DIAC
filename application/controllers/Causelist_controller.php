...
class Causelist_controller extends CI_Controller
{
	public function __construct()
	{
		...
		# models
		$this->load->model(array('common_model', 'causelist_model', 'category_model', 'case_model', 'claimant_respondent_model', 'arbitral_tribunal_model', 'master_setup/arbitrator_setup_model'));
	...
	*/
	...
	public function _remap($method)
	{
		...
		$role_action_auth = array(
			...
			'DEPUTY_COUNSEL' => array(
				'cause_list', 'fetch_hearings', 'hearings_today', 'add_cause_list', 'check_rooms_availability', 'reschedule_cause_list'
			),
		);
	...
	public function reschedule_cause_list()
	{
		$case_slug = $this->input->get('case_no');
		$id = $this->input->get('id');

		if (!$case_slug || !$id) {
			return redirect(base_url('cause-list'));
		}

		$id = base64_decode($id);

		// Get the case details 
		$data['case_details'] = $this->case_model->get(['slug' => $case_slug], 'GET_CASE_BASIC_DATA');
		$data['causelist_details'] = $this->causelist_model->get_causelist_using_id($id);
		$data['arbitrators'] = $this->arbitrator_setup_model->getArbitratorsLists($case_slug);

		if (!$data['case_details'] || !$data['causelist_details']) {
			return redirect(base_url('cause-list'));
		}
		...
		if ($page_status != 0) {
			...
			$data['page_title'] = 'Re-schedule Cause List: ' . get_full_case_number($data['case_details']);
			$data['all_rooms_list'] = $this->causelist_model->get('', 'ALL_ROOMS_LIST');
			$data['purpose_category'] = $this->category_model->get('', 'GET_ALL_PURPOSE_CATEGORY');
			// $data['all_case_list'] = $this->causelist_model->get('', 'get_all_cases_list');
			$data['arbitrators_list'] = $this->causelist_model->get(null, 'get_arbitrators_list');
			...
			$this->load->view('cause_list/re-schedule-cause-list.php', $data);
		} else {
			$this->load->view('templates/page_maintenance');
		}
		$this->load->view('templates/footer');
	...
	public function check_rooms_availability()
	{
		...
		$time_from = date('H:i', strtotime($this->input->post('time_from')));
		$time_to = date('H:i', strtotime($this->input->post('time_to')));
