...
class Service extends CI_Controller
{
	...
	public function delete_registered_case()
	{
		...
		$inputCsrfToken = $_POST['csrf_trans_token'];
		if ($inputCsrfToken != '') {
			if (checkToken($inputCsrfToken, 'dataTableCaseform') || checkToken($inputCsrfToken, 'dataTableCaseRegisteredList')) {
	...
	public function arbitral_tribunal_notification_to_everyone()
	{
		...
		if ($case_no) {
			...
			$subject = 'Arbitrator added in case :-' . get_full_case_number($case_details_data) . ' (' . $case_details_data['case_title'] . ')';
	...
	public function ransfer_case_from_one_user_to_other()
	{
		$inputCsrfToken = $_POST['csrf_trans_token'];
		if ($inputCsrfToken != '') {
			if (checkToken($inputCsrfToken, 'transfer_case_token')) {
				$config = array(
					array(
						'field'   => 'user_from',
						'label'   => 'User From',
				...
				if ($this->form_validation->run() == FALSE) {
					...
					echo json_encode($this->case_model->ransfer_case_from_one_user_to_other());
				}
			} else {
				$data = array(
					'status' => false,
