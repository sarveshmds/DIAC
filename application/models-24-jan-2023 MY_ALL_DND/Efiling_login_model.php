<?php
class Efiling_login_model extends CI_model
{
    ...
    public function login()
    {
        ...
        $this->db->select('um.user_code, um.user_name, um.user_display_name, um.job_title, um.record_status, resom.resource_link, resom.is_maintenance, um.is_new_record, udm.user_logo, rm.role_name, rm.role_code, um.fk_state_code, um.primary_role');
            ...
            if ($this->session->userdata('captchaword') == $txt_captcha) {
                if ($userDataArr[0]['record_status'] != 0) {
                    ...
                    if ($userDataArr[0]['is_new_record'] != 0) {
                        ...
                        $resource_link = 'efiling/set-password';
                    }
