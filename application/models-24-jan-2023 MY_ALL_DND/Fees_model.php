	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'CASE_FEE_DEPOSIT_LIST':
				...
				$this->db->select("fd.id, fd.case_no, DATE_FORMAT(fd.date_of_deposit, '%d-%m-%Y') date_of_deposit, fd.deposited_by, fd.name_of_depositor, fd.deposited_towards, fd.mode_of_deposit, fd.details_of_deposit, fd.amount, cl_res.name cl_res_name, cl_res.count_number cl_res_count_number, gc.description dep_by_description, gc2.description as mod_description, fd.check_bounce");
			...
			case 'ADD_CASE_FEE_COST_DETAILS':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$fee_cost_data = array(
							'case_no' => $case_no,
							'arbitral_tribunal_strength' => $this->security->xss_clean($data['arbitral_tribunal_strength']),
						...
						if ($this->input->post('hidden_id')) {
							...
							$result = $this->db->where('id', customURIDecode($this->input->post('hidden_id')))->update('cs_fee_details_tbl', $fee_cost_data);
						} else {
