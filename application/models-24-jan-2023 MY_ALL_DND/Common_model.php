...
class Common_model extends CI_Model
{
	...
	* Function to get the user details with the username
	...
	{
		$user = $this->db->select('user_code, user_name, user_display_name, job_title, primary_role')->where(array('user_code' => $user_code))->from('user_master')->get()->row_array();
	...
	public function get_all_alloted_case_list()
	{
		...
		if (in_array($this->role, FULL_ACCESS_ROLES)) {
			return $this->db->from('cs_case_details_tbl')
				->select('slug, case_no, case_title')
				->where('status', 1)
				->get()
				->result_array();
		} else {
			return $this->db->from('cs_case_allotment_tbl as cat')
				->select('cat.alloted_to, cdt.slug, cdt.case_no, cdt.case_title')
