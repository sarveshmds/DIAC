	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'CASE_ALL_FILES':
				...
				$this->db->from('cs_miscellaneous_case_mapping_tbl as cmcmt');
			...
			case 'VIEW_REGISTER_CASE_DATA':
				...
				return $this->db->select("cdt.id as cdt_id, cdt.slug as cdt_slug, cdt.case_no, cdt.case_title, cdt.reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.arbitration_petition, cdt.name_of_court, cdt.name_of_judge,cdt.recieved_on, cdt.registered_on, cdt.type_of_arbitration, cdt.di_type_of_arbitration, cdt.case_status, cdt.case_file, cdt.remarks, gc.description as case_status_dec, gc2.description as reffered_by_desc, gc3.description as type_of_arbitration_desc, cdt.created_on, cdt.updated_on, gc4.description as di_type_of_arbitration_desc, gc5.description as arbitration_status_desc, enrt.soc_document, enrt.urgency_app_document, enrt.proof_of_service_doc, enrt.document as nr_case_document")
			...
			case 'ALL_CASE_DATA':
				...
				$case_data = $this->db->select("cdt.id as cdt_id, cdt.slug as cdt_slug, cdt.case_no, cdt.case_title, cdt.reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.arbitration_petition, cdt.name_of_court, cdt.name_of_judge,cdt.recieved_on, cdt.registered_on, cdt.type_of_arbitration, cdt.case_status, cdt.remarks, gc.description as case_status_dec, gc2.description as reffered_by_desc, gc3.description as type_of_arbitration_desc, cdt.created_on, cdt.updated_on, gc4.description as di_type_of_arbitration_desc, gc5.description as arbitration_status_desc")
					->from('cs_case_details_tbl as cdt')
					->join('gen_code_desc as gc', 'gc.gen_code = cdt.case_status', 'left')
					->join('gen_code_desc as gc2', 'gc2.gen_code = cdt.reffered_by AND gc2.gen_code_group = "REFFERED_BY"', 'left')
					->join('gen_code_desc as gc3', 'gc3.gen_code = cdt.type_of_arbitration AND gc3.gen_code_group = "TYPE_OF_ARBITRATION"', 'left')
			...
			case 'CASE_ARBITRAL_TRIBUNAL_LIST':
				...
				$this->db->select("cat.id,cat.at_code, cat.arbitrator_type, cat.name_of_arbitrator, cat.email, cat.phone, cat.whether_on_panel,CASE WHEN cat.whether_on_panel = 'yes' THEN 'Yes' WHEN cat.whether_on_panel = 'no' THEN 'No' ELSE '' END as whether_on_panel_desc, cat.at_cat_id, cat.appointed_by, DATE_FORMAT(cat.date_of_appointment, '%d-%m-%Y') as date_of_appointment, DATE_FORMAT(cat.date_of_declaration, '%d-%m-%Y') as date_of_declaration, cat.arb_terminated,CASE WHEN cat.arb_terminated = 'yes' THEN 'Yes' WHEN cat.arb_terminated = 'no' THEN 'No' ELSE '' END as arb_terminated_desc, DATE_FORMAT(cat.date_of_termination, '%d-%m-%Y') as date_of_termination, cat.reason_of_termination, pct.category_name, gc.description as appointed_by_desc, gc2.description as arbitrator_type_desc, cat.at_termination_by, cat.termination_files");
				...
				$this->db->join('panel_category_tbl as pct', 'pct.id = cat.at_cat_id', 'left');
			...
			case 'CASE_COUNSELS_LIST':
				...
				$this->db->select("ct.id, ct.case_no, ct.enrollment_no, ct.name, ct.appearing_for, ct.address, ct.email, ct.phone, ct.discharge, CASE WHEN ct.discharge = 1 THEN 'Yes' WHEN ct.discharge = 0 THEN 'No' END as discharged, DATE_FORMAT(ct.date_of_discharge, '%d-%m-%Y') date_of_discharge,  cl_res.name cl_res_name, cl_res.count_number cl_res_count_number");
			...
			case 'CASE_ALLOTMENT_LIST':
				...
				$this->db->select("ca.id, ca.case_no, cdt.case_no as case_no_desc, cdt.case_title, ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, ca.user_role, rm.role_name as user_role_desc");
				...
				*/
				...
				// Filter records
				$this->db->select("ca.id, ca.case_no, cdt.case_no as case_no_desc, ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, ca.user_role, rm.role_name as user_role_desc");
				$this->db->from('cs_case_allotment_tbl as ca');
				$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ca.case_no', 'left');
				$this->db->join('user_master as um', 'um.user_code = ca.alloted_to', 'left');
				$this->db->join('role_master as rm', 'rm.role_code = ca.user_role', 'left');
			...
			case 'ALLOTTED_CASE_LIST':
				...
				$this->db->select("ca.id, ca.case_no, cdt.slug as case_slug, cdt.case_no as case_no_desc, cdt.case_title as case_title, DATE_FORMAT(cdt.registered_on, '%d-%m-%Y') as registered_on, gc.description as case_status,ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, cdt.arbitrator_status, gc2.description as arbitrator_status_desc");
				$this->db->from('cs_case_allotment_tbl as ca');
			...
			case 'ALL_RESPONSE_FORMAT':
				...
				$select_column = array("body", "subject", "email_to", "cc");
				$order_column = array(null, null, null, null);

				$this->db->select("rf.id, rf.case_no, rf.subject, rf.email_to, rf.cc, rf.bcc, rf.status, cdt.case_no as case_no_desc, DATE_FORMAT(rf.created_at, '%d-%m-%Y') as created_at_date, rf.response_status");
	...
	public function post($data, $op)
	{
		switch ($op) {
			...
			case 'ADD_CASE_ARBITRAL_TRIBUNAL':
				...
				$this->form_validation->set_rules('at_arb_type', 'Arbitrator Type', 'xss_clean');
				$this->form_validation->set_rules('at_whe_on_panel', 'Wheather on panel', 'xss_clean');
				$this->form_validation->set_rules('at_category', 'Category', 'xss_clean');
				$this->form_validation->set_rules('at_appointed_by', 'Appointed by', 'xss_clean');
					...
					try {
						...
						if ($data['at_arb_contact']) {
							$checkContact = $this->db->select('*')
								...
								->where('case_no', $case_no)
								->where('status', 1)
								->where('(phone ="' . $this->security->xss_clean($data['at_arb_contact']) . '")')
								->get()
								->num_rows();
						...
						$at_data = array(
							...
							'phone' => $this->security->xss_clean($data['at_arb_contact']),
						...
						if ($result) {
							...
							$at_mail_res = send_arbitrator_for_consent($at_data, '');
			...
			case 'EDIT_CASE_ARBITRAL_TRIBUNAL':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						// Insert case details
						$result = $this->db->where('at_code', $at_code)->update('cs_arbitral_tribunal_tbl', $at_data);
						if ($result) {
