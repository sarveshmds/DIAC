				...
				if ($result) {
					...
					foreach ($result as $key => $res) {
						...
						if ($res['type_table'] == 'miscellaneous_noting_tbl') {
							...
							// print_r($result3); die();
							if ($result3) {
								$notification_text = 'A miscellaneous is added for you by ' . $result3['marked_by_user'] . ' at ' . date('d M, Y - h:i A', strtotime($result3['marked_at']));
						...
						if ($res['type_table'] == 'miscellaneous_replies_tbl') {
							...
							// print_r($result3); die();
							if ($result3) {
								$notification_text = 'A reply is given on miscellaneous for you by ' . $result3['marked_by_user'] . ' at ' . date('d M, Y - h:i A', strtotime($result3['marked_at']));
