...
class Case_controller extends CI_Controller
{
	public function __construct()
	{
		...
		# models
		$this->load->model(array('common_model', 'case_model', 'category_model', 'fees_model', 'notification_model', 'refferal_request_model', 'panel_category_model', 'claimant_respondent_model', 'arbitral_tribunal_model', 'master_setup/arbitrator_setup_model', 'response_model', 'master_setup/minor_stages_setup_model'));
	...
	*/
	...
	public function _remap($method)
	{
		...
		$role_action_auth = array(
			...
			'HEAD_COORDINATOR' => array(
				'index', 'all_cases', 'noting', 'case_allotment', 'view_case', 'view_fees_details', 'case_all_files', 'case_fees_details', 'fees_assessment', 'view_fees_assessment', 'allotted_case'
			),
			'COORDINATOR' => array(
				'index', 'all_cases', 'noting', 'case_allotment', 'view_case', 'view_fees_details', 'case_all_files', 'case_fees_details', 'fees_assessment', 'view_fees_assessment', 'allotted_case', 'arbitral_tribunal', 'case_order'
			),
			'ACCOUNTS' => array(
			...
			'DEPUTY_COUNSEL' => array(
				'index', 'all_cases', 'status_of_pleadings', 'claimant_respondant_details', 'counsels', 'arbitral_tribunal', 'case_fees_details', 'termination', 'noting', 'view_case', 'allotted_case', 'view_fees_details', 'case_all_files', 'add_case_fee_assessment', 'case_order', 'get_arbitrators_data', 'add_counsels', 'edit_counsels'
			),
			'DEO' => array(
				'all_cases', 'claimant_respondant_details', 'counsels', 'arbitral_tribunal', 'view_case', 'allotted_case', 'view_fees_details'
			),
		);
	...
	public function case_fees_details($case_no = '')
	{
		...
		if ($page_status != 0) {
			...
			if (isset($case_no) && !empty($case_no)) {
				...
				$arb_rel_list = $this->case_model->get($case_no, 'get_arbitrators_list');
				$data['release_arbitrators_list'] = $arb_rel_list;
				// echo "<pre>";
				// print_r($data['fee_prayers']);
				// echo "</pre>";
				// die;
	...
	public function allotted_case()
	{
		...
		if ($page_status != 0) {
			...
			// print_r($data['alloted_cases_list']);
			// die;
