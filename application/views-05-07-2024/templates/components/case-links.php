...
<!-- Linked Cases -->
<?php
$linkedCases = getLinkedCases($case_details['slug']);

if (count($linkedCases)) {
    $masterCase = getLinkedMasterCase($linkedCases[0]['master_case']);
}
?>
            ...
            <div class="card-body">
                <ul class="list-unstyled case-links-list">
                    ...
                    <?php foreach ($linkedCases as $case) : ?>
                        <li>
                            <a href="<?= base_url('view-case/' . $case['tagged_case']) ?>" class="btn btn-warning btn-top-links" target="_BLANK"><i class="fa fa-paper-plane-o"></i> <?= get_full_case_number($case) ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
