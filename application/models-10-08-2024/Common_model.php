...
class Common_model extends CI_Model
{
	...
	* Function to get the user details with the username
	...
	public function get_all_users_related_to_case($case_no)
	{
		...
		$this->db->select('um.*');
		$this->db->where_in('um.primary_role', ['HEAD_COORDINATOR', 'ACCOUNTS']);
	...
	// Get all users alloted to particular case
	public function get_deputy_counsel_alloted_to_case($case_no)
	{
		$this->db->from('cs_case_allotment_tbl as cat');
		$this->db->select('um.*');
		$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug = cat.case_no', 'left');
		$this->db->join('user_master as um', 'um.user_code = cat.alloted_to', 'left');
		$this->db->where([
			'cat.user_role' => 'DEPUTY_COUNSEL',
			'cat.case_no' => $case_no,
			'cat.status' => 1
		]);
		$alloted_users = $this->db->get()->row_array();
	...
	function get_person_details_of_alloted_case($case_no, $user_type)
	{
		$this->db->select('um.user_display_name,um.job_title');
		$this->db->from('cs_case_allotment_tbl as cat');
		$this->db->join('user_master as um', 'um.user_code = cat.alloted_to', 'left');
		$this->db->where('cat.case_no', $case_no);
		$this->db->where('cat.user_role', $user_type);
		return $this->db->get()->row_array();
	}

	function get_counsels_of_case($case_no, $type_for_counsel)
	{
		...
		$this->db->where('cl_res.type', $type_for_counsel);
		$this->db->where('ct.status', 1)->where('ct.case_no', $case_no);
		$query = $this->db->get();
		return $query->result_array();
	}

	public function get_case_status_timeline($case_no)
	{
		$this->db->select('ccst.*, gc.description as case_status_desc, mmst.name as minor_case_status_name, um.user_display_name, um.job_title');
		$this->db->from('cs_case_status_timelines as ccst');
		$this->db->join('gen_code_desc as gc', 'gc.gen_code = ccst.case_status AND gc.gen_code_group = "CASE_STATUS"', 'left');
		$this->db->join('master_minor_stages_table as mmst', 'mmst.code = ccst.minor_case_status AND mmst.major_stage_code = ccst.case_status', 'left');
		$this->db->join('user_master as um', 'um.user_code = ccst.created_by', 'left');
		$this->db->where('ccst.case_no', $case_no);
		$this->db->where('ccst.record_status', 1);
		return $this->db->get()->result_array();
	}
}
