						...
						<div id="fr_wrapper">
							<div class="row">
								...
								<div class="form-group col-md-3 col-sm-6 col-xs-12 required">
									...
									<select name="counsel_appearing_for" id="counsel_appearing_for" class="form-control counsel_appearing_for">
										...
										<?php foreach ($claim_res_data as $cr_data) : ?>
											<option value="<?= $cr_data['code'] ?>" <?= isset($counsels_data['appearing_for']) && $counsels_data['appearing_for'] == $cr_data['code'] ? 'selected' : '' ?>><?= $cr_data['name'] . '(' . CLAIMANT_RESPONDENT_TYPE[$cr_data['type']] . ' - ' . $cr_data['count_number'] . ')' ?></option>
										<?php endforeach; ?>
								...
								<div class="form-group col-md-3 col-sm-6 col-xs-12 required" id="select_enrol_col">
									...
									<select class="form-control select2" name="counsels_list" id="counsels_list">
										<option value="">Select</option>
										<option value="OTHER">Other</option>
										<?php foreach ($all_counsels_list as $counsels_list) : ?>
											<option value="<?= $counsels_list['code'] ?>" <?= isset($counsels_data['name_code']) && $counsels_data['name_code'] == $counsels_list['code'] ? 'selected' : '' ?>><?= $counsels_list['name'] ?> <?= ($counsels_list['enrollment_no']) ? " (" . $counsels_list['enrollment_no'] . ")" : ''; ?></option>
										<?php endforeach ?>
									</select>
								</div>
