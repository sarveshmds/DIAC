		...
		switch ($op) {
			...
			case 'get_userdata':
				...
				if ($order) {
					...
					$this->db->order_by('a.diac_serial_no', 'ASC');
				}
			...
			case 'add_user':
				...
				$data = array(
					...
					"diac_serial_no" 			=> $this->security->xss_clean($this->input->post('diac_serial_no')),
					'case_ref_no'			=> '',
					"password" 				=> $this->security->xss_clean($this->input->post('secreatecode')),
					"record_status" 		=> $this->security->xss_clean($this->input->post('user_status')),
					"created_by" 			=> $this->user_name,
			...
			case 'sa_reset_password':
				...
				if ($update_password) {
					$this->db->trans_commit();
					$dbstatus = TRUE;
					$dbmessage = 'Password resetted and set to default password.';
