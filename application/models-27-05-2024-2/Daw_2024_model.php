...
class Daw_2024_model extends CI_Model
{
    ...
    function get_all_registrations()
    {
        ...
        $this->db->from('daw_registrations_tbl_2024 drt');
        ...
        if (isset($_POST['registration_for_gar_live']) && $_POST['registration_for_gar_live'] != "") {
            if ($_POST['registration_for_gar_live'] == 1) {
                $this->db->where("drt.registration_for_gar_live", 1);
            }
            if ($_POST['registration_for_gar_live'] == 0) {
                $this->db->where("drt.registration_for_gar_live", 0);
            }
        }

        // if (isset($_POST['registration_for_gar_live']) && $_POST['registration_for_gar_live'] == 1) {
        //     $this->db->where("drt.registration_for_gar_live",1);
        // }else{
        //     $this->db->where("drt.registration_for_gar_live",0);
        // }

        if (isset($_POST['training_session']) && !empty($_POST['training_session'])) {
            $this->db->where("drt.registration_for_training_session", $_POST['training_session']);
        }
        ...
        // Records total
        $recordsTotal = $this->db->select("*")->from('daw_registrations_tbl_2024')->count_all_results();
    ...
    public function get_registrations_export_data($filter)
    {
        ...
        if (isset($filter['training_session']) && !empty($filter['training_session'])) {
            $this->db->where('registration_for_training_session', $filter['training_session']);
        }
    ...
    public function generate_qr_code($person_data)
    {
        ...
        // echo $encoded_id;
        // die;
        $content = base_url('daw-2024/person/') . $encoded_id;
    ...
    {
        return $this->db->from('daw_session_dates_category_2024')->select('*')->where('record_status', 1)->get()->result_array();
    }
    ...
    function get_daw_all_live_sessions()
    {
        ...
        $this->db->from('daw_live_sessions_details_tbl_2024 dlsdt');
        $this->db->join('daw_session_dates_category_2024 dsdc', 'dsdc.code = dlsdt.session_date_code', 'left');
        ...
        // Records total
        $recordsTotal = $this->db->select("*")->from('daw_live_sessions_details_tbl_2024')->count_all_results();
