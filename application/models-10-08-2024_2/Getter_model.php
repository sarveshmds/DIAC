	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'GET_ALL_REGISTERED_CASES_LIST':
				...
				if (isset($_GET['date_of_award_from']) && !empty($_GET['date_of_award_from'])) {
					$this->db->join('cs_award_term_tbl as catt', 'catt.case_no = cdt.slug', 'left');
				}

				// If counsel is set
				if (isset($_GET['name_of_counsel']) && !empty($_GET['name_of_counsel'])) {
				...
				*/
				...
				if (isset($_GET['case_no']) && !empty($_GET['case_no'])) {
					$case_no = $this->security->xss_clean($_GET['case_no']);
					$this->db->like('cdt.case_no', $case_no);
				}

				// If case title is set
				if (isset($_GET['case_title']) && !empty($_GET['case_title'])) {
					$case_title = $this->security->xss_clean($_GET['case_title']);
					$this->db->like('cdt.case_title', $case_title);
				}

				// If arbitration petition number is set
				if (isset($_GET['arbitration_petition']) && !empty($_GET['arbitration_petition'])) {
					$arbitration_petition = $this->security->xss_clean($_GET['arbitration_petition']);
					$this->db->like("cdt.arbitration_petition", $arbitration_petition);
				}

				// If type of arbitration is set
				if (isset($_GET['toa']) && !empty($_GET['toa'])) {
					$toa = $this->security->xss_clean($_GET['toa']);
					$this->db->where('cdt.type_of_arbitration', $toa);
				}

				// If case status is set
				if (isset($_GET['case_status']) && !empty($_GET['case_status'])) {
					$case_status = $this->security->xss_clean($_GET['case_status']);
					$this->db->where('cdt.case_status', $case_status);
				}

				// If registered on from is set ================
				if (isset($_GET['registered_on_from']) && !empty($_GET['registered_on_from'])) {
					$registered_on_from = $this->security->xss_clean($_GET['registered_on_from']);
					$this->db->where("cdt.registered_on >=", date('Y-m-d', strtotime($registered_on_from)));
				}

				// If registered on to is set ================
				if (isset($_GET['registered_on_to']) && !empty($_GET['registered_on_to'])) {
					$registered_on_to = $this->security->xss_clean($_GET['registered_on_to']);
					$this->db->where("cdt.registered_on <=", date('Y-m-d', strtotime($registered_on_to)));
				}


				// If reffered by(court/direct) is set
				if (isset($_GET['reffered_by']) && !empty($_GET['reffered_by'])) {
					$reffered_by = $this->security->xss_clean($_GET['reffered_by']);
					$this->db->where('cdt.reffered_by', $reffered_by);
				}

				// If refered by judge is set
				if (isset($_GET['reffered_by_judge']) && !empty($_GET['reffered_by_judge'])) {
					$reffered_by_judge = $this->security->xss_clean($_GET['reffered_by_judge']);
					$this->db->like('cdt.reffered_by_judge', $reffered_by_judge);
				}

				// If name of arbitrator is set
				if (isset($_GET['name_of_arbitrator']) && !empty($_GET['name_of_arbitrator'])) {
					$name_of_arbitrator = $this->security->xss_clean($_GET['name_of_arbitrator']);
				...
				if (isset($_GET['date_of_award_from']) && !empty($_GET['date_of_award_from'])) {
					$date_of_award_from = $this->security->xss_clean($_GET['date_of_award_from']);
					...
					if (isset($_GET['date_of_award_to']) && !empty($_GET['date_of_award_to'])) {
						$date_of_award_to = $this->security->xss_clean($_GET['date_of_award_to']);
				...
				if (isset($_GET['referred_on']) && !empty($_GET['referred_on'])) {
					$referred_on = $this->security->xss_clean($_GET['referred_on']);
					$this->db->where("cdt.reffered_on", date('Y-m-d', strtotime($referred_on)));
				}

				// If referred on to is set ================
				if (isset($_GET['case_type']) && !empty($_GET['case_type'])) {
					$case_type = $this->security->xss_clean($_GET['case_type']);
					$this->db->where("cdt.di_type_of_arbitration", $case_type);
				}

				if ($_GET['sorting_by']) {
					$this->db->order_by('cdt.' . $_GET['sorting_by'], $_GET['sort_to']);
				} else {
			...
			case 'GET_CAUSE_LIST':
				...
				if ($this->input->get('room_no')) {
					$room_no_id = base64_decode($this->input->get('room_no'));
					$this->db->where('clt.room_no_id', $room_no_id);
				}

				$this->db->group_by('clt.id');
				// $this->db->order_by('clt.date', 'ASC');
			...
			case 'GET_CASE_DETAILS_USING_ID':
				$this->db->select("cdt.id, cdt.slug as cdt_slug ,cdt.case_no as case_no, cdt.case_title, cdt.case_title_claimant, cdt.case_title_respondent,, cdt.reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.arbitration_petition, cdt.name_of_court, cdt.recieved_on, cdt.registered_on, cdt.type_of_arbitration,cdt.case_status, cdt.remarks");
				$this->db->from('cs_case_details_tbl AS cdt');
			...
			case 'GET_PRAYERS_DETAILS_FOR_EDIT':
				$this->db->select('*, prayer_name as name, prayer_amount as amount');
				$this->db->from('cs_assessment_prayers_tbl');
				$this->db->where('record_status', 1);
				$this->db->where('assessment_code', $data);
				$this->db->where('type', 'CLAIM');
