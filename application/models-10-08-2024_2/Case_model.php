...
class Case_model extends CI_Model
{
	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'ALL_DIAC_USERS':
				$this->db->select('user_code, user_name, user_display_name, job_title, primary_role');
			...
			case 'ALL_CASE_LIST':
				...
				$this->db->select("cdt.id as cdt_id, cdt.slug as cdt_slug, cdt.case_no, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, cdt.case_title, DATE_FORMAT(cdt.reffered_on, '%d-%m-%Y') as reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.arbitration_petition, cdt.name_of_court, DATE_FORMAT(cdt.recieved_on, '%d-%m-%Y') as recieved_on, DATE_FORMAT(cdt.registered_on, '%d-%m-%Y') as registered_on, cdt.type_of_arbitration, cdt.case_status, cdt.case_file, cdt.remarks, gc.description as case_status_dec, gc2.description as reffered_by_desc, gc3.description as type_of_arbitration_desc, mmst.name as minor_case_status_name");
				...
				$this->db->join('master_minor_stages_table as mmst', 'mmst.code = cdt.minor_case_status AND mmst.major_stage_code = cdt.case_status', 'left');
			...
			case 'ALL_REGISTERED_CASE_LIST':
				...
				$this->db->select("cdt.id as cdt_id, cdt.slug as cdt_slug, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, cdt.case_title, DATE_FORMAT(cdt.reffered_on, '%d-%m-%Y') as reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.arbitration_petition, cdt.name_of_court,  DATE_FORMAT(cdt.recieved_on, '%d-%m-%Y') as recieved_on, DATE_FORMAT(cdt.registered_on, '%d-%m-%Y') as registered_on, cdt.type_of_arbitration, cdt.case_status, cdt.remarks, cdt.case_file, cdt.di_type_of_arbitration, cdt.arbitrator_status,  gc.description as case_status_dec, gc2.description as reffered_by_desc, gc3.description as type_of_arbitration_desc, gc4.description as arbitrator_status_desc, gc5.description as di_type_of_arbitration_desc, cdt.reference_code, CASE WHEN cdt.`case_type` = 'REFERRALS_REQUESTS' THEN mt.diary_number WHEN  `cdt`.`case_type` = 'NEW_REFERENCE' THEN enrt.diary_number ELSE 0 END AS diary_number, GROUP_CONCAT(amt.name_of_arbitrator) as amt_name_of_arbitrator, mmst.name as minor_case_status_name");
				$this->db->from('cs_case_details_tbl AS cdt');
				$this->db->join('gen_code_desc as gc', 'gc.gen_code = cdt.case_status AND gc.gen_code_group = "CASE_STATUS"', 'left');
				$this->db->join('gen_code_desc as gc2', 'gc2.gen_code = cdt.reffered_by AND gc2.gen_code_group = "REFFERED_BY"', 'left');
				$this->db->join('gen_code_desc as gc3', 'gc3.gen_code = cdt.type_of_arbitration AND gc3.gen_code_group = "TYPE_OF_ARBITRATION"', 'left');
				...
				*/
				...
				if (isset($_POST['name_of_arbitrator']) && !empty($_POST['name_of_arbitrator'])) {
					$name_of_arbitrator = $this->security->xss_clean($_POST['name_of_arbitrator']);
					$this->db->like('amt.name_of_arbitrator', $name_of_arbitrator);
			...
			case 'GET_CASE_BASIC_DATA':
				$this->db->select("cdt.*, cdt.slug as cdt_slug , crd.name as case_title_claimant_name, crd2.name as case_title_respondent_name");
				$this->db->from('cs_case_details_tbl AS cdt');
			...
			case 'GET_CASE_NUMBER_USING_SLUG':
				$this->db->select("cdt.*, gc.description as case_status_desc, mmst.name as minor_case_status_name");
				$this->db->from('cs_case_details_tbl AS cdt');
				$this->db->join('gen_code_desc as gc', 'gc.gen_code = cdt.case_status AND gc.gen_code_group = "CASE_STATUS"', 'left');
				$this->db->join('master_minor_stages_table as mmst', 'mmst.code = cdt.minor_case_status AND mmst.major_stage_code = cdt.case_status', 'left');
				$this->db->where('cdt.slug', $data['case_no']);
			...
			case 'VIEW_REGISTER_CASE_DATA':
				...
				return $this->db->select("cdt.id as cdt_id, cdt.slug, cdt.slug as cdt_slug, cdt.case_no as case_no, cdt.case_no_prefix, cdt.case_no_year, cdt.case_title, cdt.reffered_on, cdt.reffered_by, cdt.reffered_by_judge, cdt.reffered_by_court_case_type, cdt.reffered_by_court_case_year,  cdt.arbitration_petition, cdt.name_of_court, cdt.recieved_on, cdt.registered_on, cdt.type_of_arbitration, cdt.di_type_of_arbitration, cdt.arbitrator_status, cdt.case_status, cdt.case_file, cdt.remarks, gc.description as case_status_dec, gc2.description as reffered_by_desc, gc3.description as type_of_arbitration_desc, cdt.created_on, cdt.updated_on, gc4.description as di_type_of_arbitration_desc, gc5.description as arbitration_status_desc, enrt.soc_document, enrt.urgency_app_document, enrt.proof_of_service_doc, enrt.document as nr_case_document")
			...
			case 'ALL_CASE_DATA':
				...
				// Arbitral tribunals
				$this->db->select("cat.*,  amt.name_of_arbitrator as amt_name_of_arbitrator , amt.email as amt_email, amt.contact_no as amt_contact_no, amt.whether_on_panel as amt_whether_on_panel, CASE WHEN cat.whether_on_panel = 'yes' THEN 'Yes' WHEN amt.whether_on_panel = 'no' THEN 'No' ELSE '' END as whether_on_panel_desc,amt.category, cat.appointed_by, DATE_FORMAT(cat.date_of_appointment, '%d-%m-%Y') as date_of_appointment, DATE_FORMAT(cat.date_of_declaration, '%d-%m-%Y') as date_of_declaration, cat.arb_terminated,CASE WHEN cat.arb_terminated = 'yes' THEN 'Yes' WHEN cat.arb_terminated = 'no' THEN 'No' ELSE '' END as arb_terminated_desc, DATE_FORMAT(cat.date_of_termination, '%d-%m-%Y') as date_of_termination, gc.description as appointed_by_desc, gc2.description as arbitrator_type_desc,  amt.perm_address_1,amt.perm_address_2,amt.perm_country,amt.perm_state,amt.perm_pincode,amt.corr_address_1,amt.corr_address_2,amt.corr_country,amt.corr_state,amt.corr_pincode,pct2.category_name as amt_category_name, pct2.code as amt_category_code, gc3.description as is_empanelled_desc");
				...
				$this->db->join('gen_code_desc as gc3', 'gc3.gen_code = cat.whether_on_panel AND gc3.gen_code_group = "ARB_IS_EMPANELLED"', 'left');

				// $this->db->join('panel_category_tbl as pct', 'pct.code = cat.category', 'left');
				$this->db->join('panel_category_tbl as pct2', 'pct2.code = amt.category', 'left');

				$this->db->order_by('cat.id', 'DESC');
				$this->db->where('cat.status', 1)->where('cat.case_no', $data['slug']);
				$arbitral_tribunal = $this->db->get()->result_array();


				// Counsels
				$counsels = $this->db->select("ct.id, ct.case_no, mct.enrollment_no, mct.name, ct.appearing_for, mct.email, mct.phone_number, ct.discharge, CASE WHEN ct.discharge = 1 THEN 'Yes' WHEN ct.discharge = 0 THEN 'No' END as discharged, ct.date_of_discharge,  cl_res.name as cl_res_name, cl_res.count_number as cl_res_count_number, ct.created_at, ct.updated_at")
					->from('cs_counsels_tbl as ct')
					->join('master_counsels_tbl as mct', 'mct.code = ct.name_code', 'left')
					->join('cs_claimant_respondant_details_tbl as cl_res', 'cl_res.code = ct.appearing_for', 'left')
				...
				$this->db->where('clt.slug', $data['slug']);
				...
				// Case Orders Data
				$this->db->select("cot.order_given_on, cot.document, cot.created_at, cot.code, um.user_display_name, um.job_title");
				...
				$this->db->where('cot.record_status', 1)->where('case_code', $data['slug']);
				$case_orders = $this->db->get()->result_array();
			...
			case 'GET_ALL_CLAIMANT_RESPONDENT':
				...
				$claim_res_data = $this->db->select("crt.*, cnt.name as country_name,st.name as state_name")
					->from('cs_claimant_respondant_details_tbl crt')
			...
			case 'GET_ALL_CLAIMANT_RESPONDENT_WITH_ALL_DET':
				$this->db->select("crt.*, CASE WHEN crt.removed = 0 THEN 'No' ELSE 'Yes' END as removed_desc, cnt.name as perm_country_name,st.name as perm_state_name, cnt2.name as corr_country_name,st2.name as corr_state_name");
				$this->db->from('cs_claimant_respondant_details_tbl as crt');
				$this->db->join('countries as cnt', 'cnt.iso2 = crt.perm_country', 'left');
				$this->db->join('states as st', 'st.id = crt.perm_state', 'left');
				$this->db->join('countries as cnt2', 'cnt2.iso2 = crt.perm_country', 'left');
				$this->db->join('states as st2', 'st2.id = crt.perm_state', 'left');
				$this->db->where('crt.status', 1)->where('crt.case_no', $data['case_no']);
				$this->db->order_by('count_number', 'ASC');
				$query = $this->db->get();
				return  $query->result_array();
				break;

			case 'GET_ALL_CLAIMANT_RESPONDENT_WITH_COUNSEL':
				// Get all claimant and respondant of case
			...
			case 'CASE_CLAIMANT_LIST':
				...
				$select_column = array("crt.type", "crt.name", "count_number", "contact", "email", "removed");
				$order_column = array(null, "crt.name", "count_number", "contact", "email", null);
				...
				$this->db->where('crt.status', 1)->where('crt.case_no', $data['case_no'])->where('crt.type', 'claimant');
				$query = $this->db->get();
				$fetch_data = $query->result();
			...
			case 'CASE_RESPONDANT_LIST':
				...
				$this->db->where('crt.status', 1)->where('crt.case_no', $data['case_no'])->where('crt.type', 'respondant');
				$query = $this->db->get();
				$fetch_data = $query->result();
			...
			case 'CASE_COUNSELS_LIST':
				...
				$this->db->select("ct.id,ct.case_no, ct.name_code, mct.enrollment_no, mct.name, ct.appearing_for,mct.email, mct.phone_number, ct.discharge, CASE WHEN ct.discharge = 1 THEN 'Yes' WHEN ct.discharge = 0 THEN 'No' END as discharged, DATE_FORMAT(ct.date_of_discharge, '%d-%m-%Y') date_of_discharge,  cl_res.name as cl_res_name, cl_res.count_number cl_res_count_number, cl_res.type as cl_res_type, mct.perm_address_1,mct.perm_address_2,cnt.name as perm_country_name,st.name as perm_state_name,mct.perm_pincode,mct.corr_address_1,mct.corr_address_2,cnt_2.name as corr_country_name,st_2.name as corr_state_name,mct.corr_pincode,ct.code,mct.perm_country as perm_country_code,mct.perm_state as perm_state_code,mct.corr_country as corr_country_code,mct.corr_state as corr_state_code,cl_res.name as appearing_name");
			...
			case 'CASE_ORDER_LIST':
				...
				$select_column = array("order_given_on", "document", "created_at");
				$order_column = array("order_given_on", "document", "created_at");
			...
			case 'CASE_NOTING_LIST':
				...
				// $this->db->select("nt.id, nt.case_no, nt.noting, DATE_FORMAT(nt.noting_date, '%d-%m-%Y %H:%i:%s') as noting_date, DATE_FORMAT(nt.next_date, '%d-%m-%Y') as next_date,nt.marked_to, nt.marked_by, um.user_display_name as marked_to_user, um.job_title as marked_to_job_title, um2.user_display_name marked_by_user, um2.job_title as marked_by_job_title, nt.noting_file, (@row_number:=@row_number + 1) AS serial_no, gcd.description as noting_text");
				// $this->db->from('cs_noting_tbl as nt');
				// $this->db->join('user_master as um', 'um.user_code = nt.marked_to', 'left');
				// $this->db->join('user_master as um2', 'um2.user_code = nt.marked_by', 'left');
				// $this->db->join('gen_code_desc as gcd', 'gcd.gen_code = nt.noting_text_code AND gcd.gen_code_group = "NOTING_SMART_TEXT"', 'left');

				$this->db->select("nt.id, nt.case_no, nt.noting, DATE_FORMAT(nt.noting_date, '%d-%m-%Y %H:%i:%s') as noting_date, DATE_FORMAT(nt.next_date, '%d-%m-%Y') as next_date,nt.marked_to, nt.marked_by, um.user_display_name as marked_to_user, um.job_title as marked_to_job_title, um2.user_display_name marked_by_user, um2.job_title as marked_by_job_title, nt.noting_file, gcd.description as noting_text, nt.task_done, nt.task_done_on, nt.status");
				$this->db->from('cs_noting_tbl as nt');
				$this->db->join('user_master as um', 'um.user_code = nt.marked_to', 'left');
				$this->db->join('user_master as um2', 'um2.user_code = nt.marked_by', 'left');
				$this->db->join('gen_code_desc as gcd', 'gcd.gen_code = nt.noting_text_code AND gcd.gen_code_group = "NOTING_SMART_TEXT"', 'left');
			...
			case 'CASE_ALLOTMENT_USING_USER_CODE':
				$this->db->select("ca.id, ca.case_no, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, cdt.case_title, ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, ca.user_role, rm.role_name as user_role_desc");
				...
				$this->db->where('ca.alloted_to', $this->session->userdata('user_code'));
			...
			case 'CASE_ALLOTMENT_LIST_GROUPWISE':

				$select_column = array("case_no");
				$order_column = array("case_no", "alloted_to", null);

				$this->db->select("ca.id, ca.case_no, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, cdt.case_title, ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on,
						MAX(CASE WHEN ca.user_role = 'CASE_MANAGER' THEN um.user_display_name ELSE NULL END) AS case_manager,
						MAX(CASE WHEN ca.user_role = 'DEPUTY_COUNSEL' THEN um.user_display_name ELSE NULL END) AS deputy_counsel,
						MAX(CASE WHEN ca.user_role = 'COORDINATOR' THEN um.user_display_name ELSE NULL END) AS coordinator");
				$this->db->from('cs_case_allotment_tbl as ca');
				$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ca.case_no', 'left');
				$this->db->join('user_master as um', 'um.user_code = ca.alloted_to', 'left');
				$this->db->join('role_master as rm', 'rm.role_code = ca.user_role', 'left');
				...
				*/
				...
				// Group by case ID to ensure each case has only one row in the result
				$this->db->group_by('ca.case_no');
				$this->db->order_by('cdt.case_no', 'DESC');
				...
				// Records total
				$recordsTotal = $this->db->where('status', 1)->select("*")->from('cs_case_allotment_tbl')->group_by('case_no')->count_all_results();
			...
			case 'ALLOTTED_CASE_LIST':
				...
				$select_column = array("case_no");
				$order_column = array("cdt.case_no", "cdt.case_no");

				$this->db->select("ca.id, ca.case_no, cdt.slug as case_slug, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, cdt.case_title as case_title, DATE_FORMAT(cdt.registered_on, '%d-%m-%Y') as registered_on, gc.description as case_status,ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, cdt.arbitrator_status, gc2.description as arbitrator_status_desc,  cdt.type_of_arbitration, GROUP_CONCAT(amt.name_of_arbitrator) as amt_name_of_arbitrator, mmst.name as minor_case_status_name");
				$this->db->from('cs_case_allotment_tbl as ca');
				...
				$this->db->join('gen_code_desc as gc2', 'gc2.gen_code = cdt.arbitrator_status AND gc2.gen_code_group = "ARBITRATOR_STATUS"', 'left');

				$this->db->join('cs_arbitral_tribunal_tbl as cat', 'cat.case_no = ca.case_no', 'left');
				$this->db->join('master_arbitrators_tbl as amt', 'amt.code = cat.arbitrator_code', 'left');
				$this->db->join('master_minor_stages_table as mmst', 'mmst.code = cdt.minor_case_status AND mmst.major_stage_code = cdt.case_status', 'left');
				...
				*/
				...
				if (isset($_POST['case_no']) && !empty($_POST['case_no'])) {
					$case_no = $this->security->xss_clean($_POST['case_no']);
					$this->db->where('ca.case_no', $case_no);
				}
				...
				// Minor Case status
				if (isset($_POST['minor_case_status']) && !empty($_POST['minor_case_status'])) {
					$minor_case_status = $this->security->xss_clean($_POST['minor_case_status']);
					$this->db->where('cdt.minor_case_status', $minor_case_status);
				}

				// Arbitrator Status
				if (isset($_POST['arbitrator_status']) && !empty($_POST['arbitrator_status'])) {
					$arbitrator_status = $this->security->xss_clean($_POST['arbitrator_status']);
					$this->db->where('cdt.arbitrator_status', $arbitrator_status);
				}
			...
			case 'ALL_RESPONSE_FORMAT':
				...
				// Get alloted cases list
				$cases = $this->getter_model->get(null, 'GET_CASE_NUMBERS_LIST_FOR_FORMAT_RESPONSE');
				$cases_slug_array = array_column($cases, 'slug');

				$select_column = array("body");
				$order_column = array(null);

				$this->db->select("rf.*, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, DATE_FORMAT(rf.created_at, '%d-%m-%Y') as created_at_date, DATE_FORMAT(rf.updated_at, '%d-%m-%Y') as updated_at_date, um.user_display_name, um.job_title, um2.user_display_name as updated_user_display_name, um2.job_title as updated_job_title");
				$this->db->from('cs_response_format_tbl as rf');
				$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug = rf.case_no', 'left');
				$this->db->join('user_master as um', 'rf.created_by = um.user_code', 'left');
				$this->db->join('user_master as um2', 'rf.created_by = um2.user_code', 'left');
				...
				if (isset($data['case_no']) && $data['case_no'] != '') {
					$this->db->where('rf.case_no', $data['case_no']);
				} else {
					$this->db->where_in('rf.case_no', $cases_slug_array);
				}
				...
				// Filter records
				$recordsFilteredQuery = $this->db->from('cs_response_format_tbl')->select("*");
				// $this->db->where('case_no', $data['case_no']);

				if (isset($data['case_no']) && $data['case_no'] != '') {
					$this->db->where('case_no', $data['case_no']);
				} else {
					$this->db->where_in('case_no', $cases_slug_array);
				}
				$this->db->where('status', 1);
				$recordsFiltered = $recordsFilteredQuery->count_all_results();
	...
	public function post($data, $op)
	{
		switch ($op) {
				...
				try {
					...
					if ($this->form_validation->run()) {
						...
						if ($r) {
							...
							// Delete allotment
							$cs_allotment_result = $this->db->where([
								'case_no' => $case_data['cdt_slug']
							])->update('cs_case_allotment_tbl', [
								'status' => 0,
								'updated_at' => $this->date,
								'updated_by' => $this->user_code
							]);
							...
							$clResResult = $this->db->where([
								'case_no' => $case_data['cdt_slug']
							])->update('cs_claimant_respondant_details_tbl', $cl_res_data);
			...
			case 'ADD_CLAIMANT_DETAILS':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$claimant_count = $this->db->where([
							'case_no' => $case_no,
							'type' => 'claimant'
						])->from('cs_claimant_respondant_details_tbl')->count_all_results();
						...
						$claimant_details_data = array(
							...
							// Retrieve and clean permanent address fields
							'perm_address_1' => $this->security->xss_clean($this->input->post('perm_address1')),
							'perm_address_2' => $this->security->xss_clean($this->input->post('perm_address2')),
							'perm_city' => $this->security->xss_clean($this->input->post('perm_city')),
							'perm_country' => $this->security->xss_clean($this->input->post('perm_country')),
							'perm_state' => $this->security->xss_clean($this->input->post('perm_state')),
							'perm_pincode' => $this->security->xss_clean($this->input->post('perm_pincode')),

							// Retrieve and clean correspondence address fields
							'corr_address_1' => $this->security->xss_clean($this->input->post('corr_address1')),
							'corr_address_2' => $this->security->xss_clean($this->input->post('corr_address2')),
							'corr_city' => $this->security->xss_clean($this->input->post('corr_city')),
							'corr_country' => $this->security->xss_clean($this->input->post('corr_country')),
							'corr_state' => $this->security->xss_clean($this->input->post('corr_state')),
							'corr_pincode' => $this->security->xss_clean($this->input->post('corr_pincode')),
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($result) {
							...
							$table_name = 'cs_claimant_respondant_details_tbl';
							$message = 'Details of claimant ' . $claimant_name . ' of case ' . $data['case_no'] . ' is updated.';
			...
			case 'EDIT_CLAIMANT_DETAILS_FOR_REFFERAL_REQUEST':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($result) {
							...
							$table_name = 'cs_claimant_respondant_details_tbl';
							$message = 'Details of respondent ' . $respondent_name . ' of case ' . $data['case_no'] . ' is updated.';
			...
			case 'EDIT_RESPONDANT_DETAILS_FOR_REFFERAL_REQUEST':
			...
			case 'ADD_CASE_ARBITRAL_TRIBUNAL':
				...
				$this->form_validation->set_rules('at_whe_on_panel', 'Is Empanelled', 'required|xss_clean');

				if ($this->input->post('at_whe_on_panel') == 1) {
					$this->form_validation->set_rules('at_arb_name', 'Arbitrator name', 'required|xss_clean');
				}

				if ($this->input->post('at_whe_on_panel') == 2) {

					$this->form_validation->set_rules('not_emp_arb_name', 'Arbitrator name', 'required|xss_clean');

					// $this->form_validation->set_rules('at_new_arb_name', 'Arbitrator name', 'required|xss_clean');
				...
				$this->form_validation->set_rules('at_doa', 'Date of appointment', 'xss_clean');
				$this->form_validation->set_rules('at_dod', 'Date of declaration', 'xss_clean');
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$case_no = $this->input->post('hidden_case_no');
						$arbitrator_code = generateCode();

						$record_arbitrator_code = '';
						...
						if ($this->input->post('at_whe_on_panel') == 1) {
							$record_arbitrator_code = $this->input->post('at_arb_name');

							$arb_data = [
								'at_code' => $arbitrator_code,
								'whether_on_panel' => 1,
								'case_no' => $case_no,
								'arbitrator_code' => $this->input->post('at_arb_name'),
								'status' => 1,
								'created_by' => $this->user_code,
								'created_at' => $this->date
							];
						} else {
							$record_arbitrator_code = $this->input->post('not_emp_arb_name');

							$arb_data = array(
								'at_code' => $arbitrator_code,
								'whether_on_panel' => 2,
								'case_no' => $case_no,
								'arbitrator_code' => $this->input->post('not_emp_arb_name'),
								'status' => 1,
								'created_by' => $this->user_code,
								'created_at' => $this->date
								// 'name_of_arbitrator' => $this->input->post('at_new_arb_name'),
						...
						$arb_data['arbitrator_type'] = $this->security->xss_clean($this->input->post('at_arb_type'));
						$arb_data['appointed_by'] = $this->security->xss_clean($this->input->post('at_appointed_by'));
						$arb_data['date_of_appointment'] = ($this->input->post('at_doa')) ? formatDate($this->security->xss_clean($this->input->post('at_doa'))) : null;
						$arb_data['date_of_declaration'] = ($this->input->post('at_dod')) ? formatDate($this->security->xss_clean($this->input->post('at_dod'))) : null;

						if ($this->input->post('at_terminated') && $this->input->post('at_terminated') == 'yes') {
							// Upload files ==============
							$terminationFileName = [];
							if (!empty(array_filter($_FILES['cd_termination_file']['name']))) {
								...
								if ($file_result['status'] == false) {
									$this->db->trans_rollback();
									$dbstatus = false;
									$dbmessage = "File Error: " . $file_result['msg'];
									return array('status' => $dbstatus, 'msg' => $dbmessage);
								} else {
							...
							$arb_data['date_of_termination'] = ($this->input->post('at_dot')) ? formatDate($this->input->post('at_dot')) : null;
							$arb_data['reason_of_termination'] = $this->input->post('at_rot');
							$arb_data['at_termination_by'] = $this->input->post('at_termination_by');
							$arb_data['termination_files'] = (count($terminationFileName) > 0) ? json_encode($terminationFileName) : '';
						}

						// Insert case details
						$result = $this->db->insert('cs_arbitral_tribunal_tbl', $arb_data);
						if ($result) {
							...
							// // Fetch the arbitrator details
							// $arbitrator_details = $this->arbitrator_setup_model->getArbitratorDetailsUsingCode($record_arbitrator_code);
							// if (isset($arbitrator_details['email']) && !empty($arbitrator_details['email'])) {
							// 	array_push($additional_emails, $arbitrator_details['email']);
							// }

							// // print_r($arbitrator_details);
							// // die;
							...
							$message = 'A new arbitrator ' . $this->input->post('at_new_arb_name') . ' of case ' . $case_det['case_no'] . ' is added.';
							$this->common_model->update_data_logs($table_name, $table_id, $message);
			...
			case 'EDIT_CASE_ARBITRAL_TRIBUNAL':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						// Insert case details
						$result = $this->db->where('at_code', $at_code)->update('cs_arbitral_tribunal_tbl', $arb_data);
						...
						if ($result) {
							...
							$message = 'Details of arbitrator ' . $table_id . ' of case ' . $case_det['case_no'] . ' is updated.';
							$this->common_model->update_data_logs($table_name, $table_id, $message);
			...
			case 'ADD_CASE_COUNSEL':
				...
				// Counsels details
				$counsels_type = $this->input->post('counsels_list');

				if ($counsels_type == 'OTHER') {
					$this->form_validation->set_rules('hidden_case_no', 'Case number', 'required|xss_clean');
					$this->form_validation->set_rules('counsel_enroll_no', 'Enrollment Number', 'xss_clean');
					$this->form_validation->set_rules('counsel_name', 'Name', 'required|xss_clean|regex_match[/^[a-zA-Z\s.]+$/]');
					$this->form_validation->set_rules('counsel_appearing_for', 'Appearing for', 'required|xss_clean');
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($result) {
							...
							$this->db->trans_commit();
							$dbstatus = true;
							$dbmessage = "Counsel added successfully";
							return array('status' => $dbstatus, 'msg' => $dbmessage, 'redirect_url' => base_url('counsels/' . $case_no));
						} else {
							$this->db->trans_rollback();
							$dbstatus = false;
							$dbmessage = "Something went wrong.";
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($result) {
							...
							$this->db->trans_commit();
							$dbstatus = true;
							$dbmessage = "Counsel updated successfully";
							return array('status' => $dbstatus, 'msg' => $dbmessage, 'redirect_url' => base_url('counsels/' . $case_no));
						} else {
							$this->db->trans_rollback();
							$dbstatus = false;
			...
			case 'ADD_CASE_COUNSEL_FOR_REF_REQ':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($counsels_type == 'OTHER') {
							...
							$name_code = $this->security->xss_clean($data['counsels_code']);
							// Check if the counsel is already registered in this case or not
							if ($data['counsels_code']) {
								$checkContact = $this->db->select('*')
									->from('cs_counsels_tbl')
									->where('case_no', $case_no)
						...
						$counsel_data = array(
							...
							'case_no' => $case_no,
							'name_code' => $name_code,
							'appearing_for' => $this->input->post('counsel_appearing_for'),
							'discharge' => 0,
							'date_of_discharge' => null,
							'status' => 1,
							'created_by' => $this->user_code,
			...
			case 'UPDATE_CASE_COUNSEL_FOR_REF_REQ':
				// Counsels details
				$counsels_type = $this->security->xss_clean($data['counsels_list']);
				if ($counsels_type == 'OTHER') {
					$this->form_validation->set_rules('hidden_case_no', 'Case number', 'required|xss_clean');
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if ($counsels_type == 'OTHER') {
							...
							$name_code = generateCode();
							// Check if the email / phone / enrollment number already available or not
							if ($data['counsel_enroll_no']) {
								$checkContact = $this->db->select('*')
									->from('master_counsels_tbl')
									->where('record_status', 1)
							...
							if ($data['counsel_email']) {
								...
								if ($checkContact > 0) {
									$output = array(
										'status' => 'validationerror',
										'msg' => 'Email id is already registered. Please use another contact details.'
									);
									return $output;
								}
							}
							...
							$master_counsel_data = array(
								'code' => $name_code,
								'name' => $this->input->post('counsel_name'),
								'enrollment_no' => $this->security->xss_clean($data['counsel_enroll_no']),
								'email' => $this->security->xss_clean($data['counsel_email']),
							...
							$counsel_data = array(
								'name_code' => $this->input->post('counsels_list'),
								'appearing_for' => $this->input->post('counsel_appearing_for'),
								'discharge' => 0,
								'date_of_discharge' => null,
								'status' => 1,
						...
						if ($result) {
							...
							$case_det = $this->common_model->get_case_details_from_slug($case_no);
							$table_name = 'cs_counsels_tbl';
							$message = 'Details of counsel ' . $this->input->post('counsel_name') . ' of case ' . $case_det['case_no'] . ' is updated.';
							$this->common_model->update_data_logs($table_name, $table_id, $message);
			...
			case 'ADD_CASE_ORDER':
				$this->form_validation->set_rules('order_given_on', 'Order Given On', 'required|xss_clean');
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$noting_data = array(
							...
							'order_given_on' => $this->security->xss_clean(date('Y-m-d', strtotime($data['order_given_on']))),
							'created_by' => $this->user_code,
							'created_at' => $this->date,
						...
						if ($result) {
							$this->db->trans_commit();
							$dbstatus = true;
							$dbmessage = "Order file uploaded successfully";
						} else {
							$this->db->trans_rollback();
							$dbstatus = false;
							$dbmessage = "Something went wrong.";
			...
			case 'DELETE_CASE_COUNSEL':
				...
				if ($this->form_validation->run()) {
					...
					$r = $this->db->where('code', $code)->update('cs_counsels_tbl', [
						'status' => 0,
						'updated_by' => $this->user_code,
						'updated_at' => $this->date
					]);
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$noting_data = array(
							...
							'noting_date' => currentDateTimeStamp(),
				...
				if ($this->role == 'DIAC') {
					...
					if ($this->form_validation->run()) {
						...
						try {
							...
							$noting_id = $this->security->xss_clean($data['id']);
							...
							// Update case noting details
							$result = $this->db->where('id', $noting_id)->update('cs_noting_tbl', $noting_data);
							...
							if ($result) {
								...
								$user_details = $this->common_model->get_user_details_using_usercode($this->user_code);

								$table_name = 'cs_noting_tbl';
								$table_id = $noting_id;
								$message = 'Noting is deleted in the case no. ' . $case_det['case_no'] . ' by ' . $user_details['user_name'];
								$this->common_model->update_data_logs($table_name, $table_id, $message);
			...
			case 'TASK_DONE_CASE_NOTING_UPDATE':

				$this->form_validation->set_rules('case_code', 'Case number', 'required|xss_clean');
				$this->form_validation->set_rules('id', 'ID', 'required|xss_clean');
				$this->form_validation->set_rules('task_done', 'Task Done', 'required|xss_clean');
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$case_no = $this->input->post('case_code');

						$noting_data = array(
							'task_done' => $this->input->post('task_done'),
							'task_done_on' => currentDateTimeStamp(),
							'updated_by' => $this->user_code,
							'updated_at' => currentDateTimeStamp(),
						);
						...
						$result = $this->db->where([
							'case_no' => $case_no,
							'id' => $id,
						])->update('cs_noting_tbl', $noting_data);
						...
						if ($result) {
							...
							$table_name = 'cs_noting_tbl';
							$table_id = $id;
							$message = 'Task done marked by the user in the case:' . $case_det['case_no'];
							$this->common_model->update_data_logs($table_name, $table_id, $message);
			...
			case 'ADD_CASE_ALLOTMENT':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						$ca_allotted_to_coord = (isset($data['ca_allotted_to_coord'])) ? $this->security->xss_clean($data['ca_allotted_to_coord']) : [];
						// $allotment_role = $this->security->xss_clean($data['ar_role']);

						if (count($ca_allotted_to_dc) < 1 && count($ca_allotted_to_cm) < 1 && count($ca_allotted_to_coord) < 1) {
							$this->db->trans_rollback();
							return array(
								'status' => false,
						...
						if (count($ca_allotted_to_coord) > 0) {
							foreach ($ca_allotted_to_coord as $key => $user) {
								// Check if the case is already alloted or not
								// if alloted, return with error message
								$res = $this->common_model->check_case_allotted($case_no, $user);
			...
			case 'EDIT_CASE_ALLOTMENT':
				...
				if ($this->form_validation->run()) {
					...
					try {
						...
						if (isset($data['ca_allotted_to_coord'])) {
							$alloted_to = $this->security->xss_clean($data['ca_allotted_to_coord'][0]);
						}
			...
			case 'CHANGE_CASE_STATUS':
				$this->form_validation->set_rules('hidden_case_no', 'Case Number', 'required|xss_clean');
				$this->form_validation->set_rules('major_case_stage', 'Major Case Stage', 'required|xss_clean');
				$this->form_validation->set_rules('minor_case_stage', 'Minor Case Stage', 'required|xss_clean');
				$this->form_validation->set_rules('status_changed_on', 'Status Changed On', 'required|xss_clean');

				if ($this->form_validation->run()) {
					$this->db->trans_begin();
					try {
						$hidden_case_no = $this->input->post('hidden_case_no');

						$data = array(
							'case_status' => $this->input->post('major_case_stage'),
							'minor_case_status' => $this->input->post('minor_case_stage'),
							'case_status_remarks' => $this->input->post('remarks'),
							'status_changed_on' => formatDate($this->input->post('status_changed_on')),
							'updated_by' => $this->user_code,
							'updated_on' => $this->date
						);

						$result = $this->db->where('slug', $hidden_case_no)->update('cs_case_details_tbl', $data);
						if ($result) {

							// Add the record in case status timelines
							$cst_data = [
								'case_no' => $hidden_case_no,
								'case_status' => $this->input->post('major_case_stage'),
								'minor_case_status' => $this->input->post('minor_case_stage'),
							...
							$cst_result = $this->db->insert('cs_case_status_timelines', $cst_data);

							if ($cst_result) {
								$table_name = 'cs_case_details_tbl';
								$table_id = $hidden_case_no;
								...
								$output = array(
									'status' => false,
									'msg' => 'Error while adding data into status timeline'
								);
							}
						} else {
							$this->db->trans_rollback();
	...
	public function getCaseDetailsUsingCaseCode($case_code)
	{
		...
		$this->db->from('cs_case_details_tbl as cdt ');
		$this->db->join('miscellaneous_tbl as m', 'cdt.reference_code = m.m_code', 'left');
		...
		$this->db->where('cdt.slug', $case_code);
		$this->db->where('cdt.status', 1);
		$query = $this->db->get();
		return $query->row_array();
	}

	public function re_allocate_all_cases()
	{
		$this->db->trans_begin();
		try {
			...
			$query = $this->db->query("
					UPDATE cs_case_details_tbl cd INNER JOIN  master_case_ref_no_tbl mct ON  mct.type='" . $type . "' AND
					start_index<= CAST(REPLACE(SUBSTRING(cd.case_no,-2),'00','100') AS UNSIGNED)
				AND REPLACE(end_index,'00','100')>=CAST(REPLACE(SUBSTRING(cd.case_no,-2),'00','100') AS UNSIGNED) AND cd.case_status IN ('UNDER_PROCESS','UNDER_HEARING', 'AWARD_TERMINATION') 
					INNER JOIN user_master um ON mct.code=um.case_ref_no AND um.primary_role='" . $role . "' AND um.record_status = 1
					INNER JOIN cs_case_allotment_tbl cst ON slug=cst.case_no AND cst.user_role='" . $role . "'
					SET cst.alloted_to= um.user_code
					WHERE  mct.type='" . $type . "' AND
					start_index<= CAST(REPLACE(SUBSTRING(cd.case_no,-2),'00','100') AS UNSIGNED)
				AND REPLACE(end_index,'00','100')>=CAST(REPLACE(SUBSTRING(cd.case_no,-2),'00','100') AS UNSIGNED) 
				AND cd.case_status IN ('UNDER_PROCESS','UNDER_HEARING', 'AWARD_TERMINATION') AND mct.code=um.case_ref_no AND um.primary_role='" . $role . "' AND
				slug=cst.case_no AND cst.user_role='" . $role . "' AND um.record_status = 1		   
			");

			// Check for affected rows to ensure the query was successful
			if ($this->db->affected_rows() > 0) {
				$this->db->trans_commit();
				$output = array(
					'status' => true,
					'msg' => 'Re-allocation successfully completed'
				);
				return $output;
			} else {
				$this->db->trans_rollback();
				$output = array(
					'status' => false,
					'msg' => 'No records are available to update. All records are already updated.'
				);
				return $output;
			}
		} catch (Exception $e) {
	...
	public function ransfer_case_from_one_user_to_other()
	{
		$this->db->trans_begin();
		try {
			...
			// Get all the alloted case from user_from
			$cases = $this->db->from('cs_case_allotment_tbl')->select('*')->where([
				'alloted_to' => $user_from,
				'user_role' => $user_type
			])->get()->result_array();
			...
			if (count($cases) > 0) {
				...
				if ($result) {
					...
					$output = array(
						'status' => true,
						'msg' => 'Transfer cases from one user to another successfully completed'
					);
					return $output;
				} else {
					$this->db->trans_rollback();
					$output = array(
						'status' => false,
						'msg' => 'No records are available to update.'
					);
					return $output;
				}
			}
	...
	// public function get_case_status_wise_cases(){
	// 	$this->db->from('cs_case_status_timelines as ccst');
	// 	$this->db->select('ccst.*');
	// 	$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug', 'ccst.case_no', 'left');
	// 	$this->db->where('cdt.case_status', 'UNDER_PROCESS');
	// 	$this->db->order_by('ccst.id', 'DESC');
	// }
}
