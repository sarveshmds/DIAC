                        ...
                        <div class="nav-tabs-custom">
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane" id="tab_2">
                                    <table id="arbitrator_setup_datatable_unappr" class="table table-condensed table-striped table-bordered dt-responsive" data-page-size="10">
                                        <input type="hidden" name="csrf_trans_token" id="arb_form_token" value="<?php echo generateToken('arbitrator_setup_datatable'); ?>">
                                        <thead>
                                            <tr>
                                                <th style="width:8%;">S. No.</th>
...
<script>
    ...
    var arbitrator_setup_datatable = $('#arbitrator_setup_datatable_appr').DataTable({
        ...
        "columns": [{
            ...
            // }, 
            {
                ...
                "sClass": "alignCenter",
                "render": function(data, type, row, meta) {
                    return '<button class="btn btn-warning btn-sm" onclick="btn_edit_appr(event)" data-tooltip="tooltip" title="Edit Details"><span class="fa fa-edit"></span></button> <button class="btn btn-danger btn-sm" onclick="delete_case_ref_no(' + data.id + ')" data-tooltip="tooltip" title="Delete"><span class="fa fa-trash"></span></button>'
                }
            }
    ...
    // datatable for Unapproved arbitrator
    var arbitrator_setup_datatable = $('#arbitrator_setup_datatable_unappr').DataTable({
        "processing": true,
        "serverSide": true,
        "autoWidth": false,
        "responsive": true,
        "order": [],
        "ajax": {
            url: base_url + "arbitrator-setup/get-datatable-data_unappr",
            type: 'POST',
            data: {
                csrf_trans_token: $('#arb_form_token').val()
            }
        ...
        "columns": [{
            ...
            // }, 
            {
                ...
                "sClass": "alignCenter",
                "render": function(data, type, row, meta) {
                    return '<button class="btn btn-warning btn-sm" onclick="btn_edit_unappr(event)" data-tooltip="tooltip" title="Edit Details"><span class="fa fa-edit"></span></button> <button class="btn btn-danger btn-sm" onclick="delete_case_ref_no(' + data.id + ')" data-tooltip="tooltip" title="Delete"><span class="fa fa-trash"></span></button>'
                }
            }
    ...
    function btn_edit_unappr(event) {
        ...
        // Get data table instance to get the data through row
        var oTable = $('#arbitrator_setup_datatable_unappr').dataTable();
        var row;
        if (event.target.tagName == "BUTTON")
            row = event.target.parentNode.parentNode;
        else if (event.target.tagName == "SPAN")
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (obj.status == true) {
                        ...
                        arbitrator_setup_datatable_appr = $('#arbitrator_setup_datatable_appr').DataTable();
                        arbitrator_setup_datatable_appr.draw();
                        arbitrator_setup_datatable_appr.clear();

                        arbitrator_setup_datatable_unappr = $('#arbitrator_setup_datatable_unappr').DataTable();
                        arbitrator_setup_datatable_unappr.draw();
                        arbitrator_setup_datatable_unappr.clear();
