	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'CASE_ALLOTMENT_LIST_GROUPWISE':
				...
				// Filter records
				$this->db->select("ca.id, ca.case_no, CONCAT(cdt.case_no_prefix,'/', cdt.case_no, '/', cdt.case_no_year) as case_no_desc, ca.alloted_to, ca.alloted_by, um.user_display_name as alloted_to_name, DATE_FORMAT(ca.created_at, '%d-%m-%Y') as alloted_on, ca.user_role, rm.role_name as user_role_desc");
				$this->db->from('cs_case_allotment_tbl as ca');
				$this->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ca.case_no', 'left');
				$this->db->join('user_master as um', 'um.user_code = ca.alloted_to', 'left');
				$this->db->join('role_master as rm', 'rm.role_code = ca.user_role', 'left');
	...
	public function re_allocate_all_cases()
	{
		...
		try {
			...
			$query = $this->db->query("
				...
				AND REPLACE(end_index,'00','100')>=CAST(REPLACE(SUBSTRING(cd.case_no,-2),'00','100') AS UNSIGNED) AND cd.case_status IN ('UNDER_PROCESS','UNDER_HEARING') 
				...
				AND cd.case_status IN ('UNDER_PROCESS','UNDER_HEARING') AND mct.code=um.case_ref_no AND um.primary_role='" . $role . "' AND
