	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'CASE_COUNSELS_LIST':
				...
				$this->db->select("ct.id,ct.case_no, mct.enrollment_no, mct.name, ct.appearing_for,mct.email, mct.phone_number, ct.discharge, CASE WHEN ct.discharge = 1 THEN 'Yes' WHEN ct.discharge = 0 THEN 'No' END as discharged, DATE_FORMAT(ct.date_of_discharge, '%d-%m-%Y') date_of_discharge,  cl_res.name as cl_res_name, cl_res.count_number cl_res_count_number, cl_res.type as cl_res_type, mct.perm_address_1,mct.perm_address_2,cnt.name as perm_country_name,st.name as perm_state_name,mct.perm_pincode,mct.corr_address_1,mct.corr_address_2,cnt_2.name as corr_country_name,st_2.name as corr_state_name,mct.corr_pincode,ct.code,mct.perm_country as perm_country_code,mct.perm_state as perm_state_code,mct.corr_country as corr_country_code,mct.corr_state as corr_state_code,cl_res.name as appearing_name");
