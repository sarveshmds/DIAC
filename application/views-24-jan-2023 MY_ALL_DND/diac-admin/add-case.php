							...
							<div class="row">
								...
								<div class="form-group col-md-3 col-sm-6 col-xs-12 required">
									...
									<select name="cd_arbitrator_status" id="cd_arbitrator_status" class="form-control">
										<option value="">Select Option</option>
										<?php foreach ($get_arbitrator_status as $as) : ?>
											<option value="<?= $as['gen_code'] ?>" <?= (isset($case_data['arbitrator_status']) && $as['gen_code'] == $case_data['arbitrator_status']) ? 'selected' : '' ?>><?= $as['description'] ?></option>
										<?php endforeach; ?>
									</select>
									<?= (isset($case_data['arbitrator_status'])) ? '<small class="text-muted">Note: If you will change appointed to, To be appointed, then it will remove all the arbitrators.</small>' : '' ?>
								</div>

								<!-- Show list of arbitrator -->
								<?php if (isset($case_data['arbitrator_status']) && $case_data['arbitrator_status'] == 1 && count($case_arbitrators_list) > 0) : ?>
									<div class="form-group col-md-3 col-sm-6 col-xs-12 required">
										<label for="" class="control-label">Name of Arbitrator (s):</label>
										<div style="display: flex; justify-content: space-between; align-items: center;">
											<div>
												<?php if (count($case_arbitrators_list) > 0) : ?>
													<?php foreach ($case_arbitrators_list as $cal) : ?>
														<p class="">* <?= $cal['name_of_arbitrator'] . (($cal['category_name']) ? ' (' . $cal['category_name'] . ')' : '') ?></p>
													<?php endforeach; ?>
												<?php else : ?>
													<p>No arbitrator is added.</p>
												<?php endif; ?>
											</div>
											<a href="<?= base_url('arbitral-tribunal/' . $case_data['cdt_slug']) ?>" class="btn btn-primary btn-xs">
								...
								<?php if (isset($case_data['arbitrator_status']) && $case_data['arbitrator_status'] == 1 && count($case_arbitrators_list) > 0) : ?>
								<?php else : ?>
									<div class="form-group col-md-6 col-sm-6 col-xs-12 name-of-arb-col" style="display: none;">
										<label for="" class="control-label" style="display: flex; justify-content: space-between;">
											Name of arbitrator (s)
											<button type="button" id="btn-add-noj" class="btn btn-success btn-xs">
												<i class="fa fa-plus"></i>
											</button>
										</label>
										<div class="noj-col-wrapper">
										</div>
									</div>
								<?php endif; ?>
