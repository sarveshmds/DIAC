        ...
        foreach ($hearings as $key => $hearing) {
            ...
            if (count($all_phone_numbers) > 0) {
                ...
                foreach ($all_phone_numbers as $key => $phone_number) {
                    ...
                    $whatsapp_result = wa_case_hearing_reminder_notification($phone_number, $full_case_no, $case_details_data, $hearing_data);
            ...
            $email_html_body = $this->load->view('emails/hearing_reminder_notice', $view_data, true);
            ...
            $insert_mail_log = [
                ...
                "status"             => (isset($email_send_result['status']) && $email_send_result['status']) ? 'SUCCESS' : 'ON_HOLD', //$output['msg']
                "created_by"         => 'CRON',
                "created_at"         => currentDateTimeStamp()
            ];
            $result_mail = $this->db->insert('support_mail_record', $insert_mail_log);

            if (isset($email_send_result['status']) && $email_send_result['status']) {
                // ============================================
