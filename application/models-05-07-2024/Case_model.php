	...
	public function get($data, $op)
	{
		switch ($op) {
			...
			case 'CASE_NOTING_LIST':
				...
				$this->db->select("nt.id, nt.case_no, nt.noting, DATE_FORMAT(nt.noting_date, '%d-%m-%Y %H:%i:%s') as noting_date, DATE_FORMAT(nt.next_date, '%d-%m-%Y') as next_date,nt.marked_to, nt.marked_by, um.user_display_name as marked_to_user, um.job_title as marked_to_job_title, um2.user_display_name marked_by_user, um2.job_title as marked_by_job_title, nt.noting_file, gcd.description as noting_text");
				$this->db->from('cs_noting_tbl as nt');
				$this->db->join('user_master as um', 'um.user_code = nt.marked_to', 'left');
				$this->db->join('user_master as um2', 'um2.user_code = nt.marked_by', 'left');
				$this->db->join('gen_code_desc as gcd', 'gcd.gen_code = nt.noting_text_code AND gcd.gen_code_group = "NOTING_SMART_TEXT"', 'left');
