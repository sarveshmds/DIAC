...
class Arbitral_tribunal_model extends CI_Model
{
    ...
    public function getBasicArbitratorData($case_code)
    {
        ...
        $this->db->select('catt.at_code, catt.whether_on_panel, catt.case_no, catt.arb_terminated, mat.name_of_arbitrator as mat_name_of_arbitrator, mat.contact_no, mat.email, mat.name_of_arbitrator, mat.perm_address_1, mat.perm_address_2, mat.perm_pincode, cnt.name as country_name, st.name as state_name');
        ...
        $this->db->join('master_arbitrators_tbl as mat', 'mat.code = catt.arbitrator_code', 'left');
        $this->db->join('countries as cnt', 'cnt.iso2 = mat.perm_country', 'left');
        $this->db->join('states as st', 'st.id = mat.perm_state', 'left');

        $this->db->where('mat.approved', 1);
        $this->db->where('catt.status', 1);
        $this->db->where('catt.case_no', $case_code);
        // $this->db->where('catt.arb_terminated !=', 'yes');
