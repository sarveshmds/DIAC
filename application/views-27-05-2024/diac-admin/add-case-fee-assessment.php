                                            ...
                                            <div class="row">
                                                    ...
                                                    <div id="claimant_claim_amount_det" class="claimant_claim_amount_det">
                                                        ...
                                                        <div class="claimant_claim_amount_form_col">
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <input type="number" name="claimant_prayer_amount" id="claimant_prayer_amount" class="form-control" placeholder="Prayer Amount" />
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="control-label">Is Quantified:</label>
                                                                <div class="form-control"><input type="checkbox" name="is_quantified" id="is_quantified" class="form-check-input" value="1" /></div>
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="control-label"></label>
                                                                <button class="btn btn-success form-control" type="button" id="btn_claimant_claim_save">
                                                ...
                                                <div class="form-group col-md-12  col-sm-12 col-xs-12 fc_sum_despute_yes_col" <?= (isset($assessment['c_cc_asses_sep']) && $assessment['c_cc_asses_sep'] == 'yes') ? '' : 'style="display: none;"' ?>>
                                                    ...
                                                    <div id="respondent_claim_amount_det" class="respondent_claim_amount_det">
                                                        ...
                                                        <div class="respondent_claim_amount_form_col">
                                                            <div class="form-group">
                                                                <label class="control-label">Prayeres Name:</label>
                                                                <input type="text" name="respondent_prayer_name" id="respondent_prayer_name" class="form-control" placeholder="Prayer Name" />
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="control-label">Prayers Amount:</label>
                                                                <input type="number" name="respondent_prayer_amount" id="respondent_prayer_amount" class="form-control" placeholder="Prayer Amount" />
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="control-label">Is Quantified:</label>
                                                                <div class="form-control"><input type="checkbox" class="form-check-input" name="res_is_quantified" id="res_is_quantified" value="1" /></div>
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="control-label"></label>
                                                                <button class="btn btn-success form-control" type="button" id="btn_respondent_claim_save">
                                                                    <i class="fa fa-save"></i> Save
                                                                </button>
                                                            </div>
                                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // let claimant_prayer_row_count_array = [];
        // let claimant_prayer_row_count = 1;
        // let respondent_prayer_row_count_array = [];
        // let respondent_prayer_row_count = 1;

        // function generate_claimant_prayer_row(claimant_prayer_row_count) {

        //     // Generate the prayers amount
        //     let prayer_html = `<div class="claimant_claim_amount_row" id="claimant_claim_amount_row_${claimant_prayer_row_count}">
        //             <input type="number" class="form-control" name="claimant_prayer_${claimant_prayer_row_count}" placeholder="Prayer amount" />
        //             <input type="text" class="form-control" name="claimant_prayer_name_${claimant_prayer_row_count}" placeholder="Prayer name" />
        //             <button type="button" class="btn btn-danger btn_remove_claimant_row" data-row-count="${claimant_prayer_row_count}" title="Delete"><i class="fa fa-trash"></i></button>
        //         </div>`;

        //     $('#claimant_claim_amount_det').append(prayer_html);

        //     claimant_prayer_row_count_array.push(claimant_prayer_row_count);
        // }

        // function generate_respondent_prayer_row(respondent_prayer_row_count) {

        //     // Generate the prayers amount
        //     let prayer_html = `<div class="respondent_claim_amount_row" id="respondent_claim_amount_row_${respondent_prayer_row_count}">
        //             <input type="number" class="form-control" name="respondent_prayer_${respondent_prayer_row_count}" placeholder="Prayer amount" />
        //             <input type="text" class="form-control" name="respondent_prayer_name_${respondent_prayer_row_count}" placeholder="Prayer name" />
        //             <button type="button" class="btn btn-danger btn_remove_respondent_row" data-row-count="${respondent_prayer_row_count}" title="Delete"><i class="fa fa-trash"></i></button>
        //         </div>`;

        //     $('#respondent_claim_amount_det').append(prayer_html);

        //     respondent_prayer_row_count_array.push(respondent_prayer_row_count);
        // }


        // // CLAIMANT PRAYER AMOUNT
        // // On Click add claimant amount row ==================
        // $('#btn_add_claimant_prayer_row').on('click', function() {
        //     claimant_prayer_row_count = claimant_prayer_row_count + 1;
        //     generate_claimant_prayer_row(claimant_prayer_row_count);
        // })

        // // On Click remove claimant amount row ==================
        // $(document).on('click', '.btn_remove_claimant_row', function() {
        //     console.log(claimant_prayer_row_count_array)
        //     if (claimant_prayer_row_count_array.length > 1) {
        //         let row_count = $(this).data('row-count');
        //         claimant_prayer_row_count_array = claimant_prayer_row_count_array.filter(function(item) {
        //             return item !== row_count
        //         })
        //         $('#claimant_claim_amount_row_' + row_count).remove();
        //     } else {
        ...
        // // RESPONDENT PRAYER AMOUNT
        // // On Click add respondent amount row ==================
        // $('#btn_add_respondent_prayer_row').on('click', function() {
        //     respondent_prayer_row_count = respondent_prayer_row_count + 1;
        //     generate_respondent_prayer_row(respondent_prayer_row_count);
        // })

        // // On Click remove respondent amount row ==================
        // $(document).on('click', '.btn_remove_respondent_row', function() {
        //     console.log(respondent_prayer_row_count_array)
        //     if (respondent_prayer_row_count_array.length > 1) {
        //         let row_count = $(this).data('row-count');
        //         respondent_prayer_row_count_array = respondent_prayer_row_count_array.filter(function(item) {
        //             return item !== row_count
        //         })
        //         $('#respondent_claim_amount_row_' + row_count).remove();
        //     } else {
        //         toastr.error('You cannot remove last remaining row from table.')
        //     }
                ...
                claimant_prayers.forEach((cp) => {
                    cp_html += `
                        <div class="claimant_claim_amount_list_row">
                            <p class="mb-0"><strong>${cp.name} :</strong> ${cp.amount} <strong style="margin-left: 50px;">Is Quantified :</strong> ${(cp.is_quantified ==1?'Yes':'No')}</p>
                            <button type="button" class="btn btn-danger btn-sm btn_claimant_prayer_remove" data-id="${cp.id}"><i class="fa fa-trash"></i></button>
                        </div>
                    `;
                })
                ...
                respondent_prayers.forEach((cp) => {
                    ...
                    cp_html += `
                        <div class="respondent_claim_amount_list_row">
                            <p class="mb-0"><strong>${cp.name} :</strong> ${cp.amount} <strong style="margin-left: 50px;">Is Quantified: </strong>${(cp.res_is_quantified ==1?'Yes':'No')}</p>
                            <button type="button" class="btn btn-danger btn-sm btn_respondent_prayer_remove" data-id="${cp.id}"><i class="fa fa-trash"></i></button>
                        </div>
                    `;
        ...
        $('#btn_claimant_claim_save').on('click', function() {
            let claimant_prayer_amount = $('#claimant_prayer_amount').val();
            let claimant_prayer_name = $('#claimant_prayer_name').val();
            let is_quantified = $('#is_quantified').val();

            if (claimant_prayer_amount && claimant_prayer_name) {
                ...
                toastr.error('Prayer name and prayer amount both fields are required.')
            }
        })
        ...
        $('#btn_respondent_claim_save').on('click', function() {
            let respondent_prayer_amount = $('#respondent_prayer_amount').val();
            let respondent_prayer_name = $('#respondent_prayer_name').val();
            let res_is_quantified = $('#res_is_quantified').val();

            if (respondent_prayer_amount && respondent_prayer_name) {
                respondent_prayers.push({
                    id: Math.ceil(Math.random() * 100) + '_' + new Date().getTime(),
                    name: respondent_prayer_name,
                    amount: respondent_prayer_amount,
                    res_is_quantified: res_is_quantified,
                })
