...
class Dashboard_controller extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model(['dashboard_model', 'Getter_model', 'fees_model', 'case_model', 'work_status_model', 'common_model']);
	...
	public function diac_dashboard()
	{
		...
		if ($page_status != 0) {
			// if (in_array($this->session->userdata('role'), ['DEPUTY_COUNSEL', 'CASE_MANAGER', 'COORDINATOR'])) {
			// 	$data['alloted_cases_list'] = $this->common_model->get_all_alloted_case_list();

			// 	// Get the case details with the status
			// }
	...
	public function steno_dashboard()
	{
		$sidebar['menu_item'] = 'Dashboard';
		$sidebar['menu_group'] = 'Dashboard';
		...
		if ($page_status != 0) {
			$this->load->view('dashboard/steno_dashboard');
		} else {
			$this->load->view('templates/page_maintenance');
		}
		$this->load->view('templates/footer');
