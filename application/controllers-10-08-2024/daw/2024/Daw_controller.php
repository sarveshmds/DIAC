...
class Daw_controller extends CI_Controller
{
	public function __construct()
	{
		...
		# models
		$this->load->model(array('daw_2024_model'));
	...
	*/
	...
	{
		$sidebar['menu_item'] = 'DAW 2024 Sessions';
		$sidebar['menu_group'] = 'DAW 2024 Sessions';
		...
		$data['page_title'] = 'DAW 2024 Sessions';

		if ($page_status != 0) {

			$data['session_dates'] = $this->daw_2024_model->get_session_dates();

			$this->load->view('daw2024/backend/all_daw_sessions', $data);
		} else {
	...
	public function view_registration_details()
	{
		...
		if ($id) {
			...
			if ($page_status != 0) {
				$data['registration_data'] = $this->daw_2024_model->get_single_registration_data($id);

				$data['page_title'] = 'Reg. No.: ' . $data['registration_data']['reg_number'];

				$this->load->view('daw2024/backend/view_registration_details', $data);
			} else {
				$this->load->view('templates/page_maintenance');
			}
			$this->load->view('templates/footer');
	...
	public function all_registrations()
	{
		...
		if ($page_status != 0) {
			...
			$data['registrant_categories'] = $this->daw_2024_model->get_gen_code_data('REGISTRANT_CATEGORY');
			$data['countries'] = $this->daw_2024_model->get_countries();

			$this->load->view('daw2024/backend/all_registrations', $data);
		} else {
			$this->load->view('templates/page_maintenance');
		}
		$this->load->view('templates/footer');
	...
	{
		echo json_encode($this->daw_2024_model->get_all_registrations());
	}
	...
	public function generate_registrations_pdf()
	{
		...
		try {
			...
			$data['page_title'] = 'DAW Registrations - 2024';
			...
			$print_data = array();
			$data['persons'] = $this->daw_2024_model->get_registrations_export_data($filter_data['filter_data']);
			...
			foreach ($data['persons'] as $row) {
				$data['person'] = $this->daw_2024_model->get_single_registration_data($row['id']);

				$data['person_qr_code'] = $this->daw_2024_model->generate_qr_code($data['person']);

				$print_data[] = $this->load->view('daw2024/export/daw_persons_registrations', $data, true);
			}

			$pdf_data = $this->load->view('daw2024/export/header', $data, true);
			$pdf_data .= implode('', $print_data);
			$pdf_data .= $this->load->view('daw2024/export/footer', $data, true);
	...
	public function generate_registrations_excel()
	{
		...
		$persons_data = $this->daw_2024_model->get_registrations_export_data($filter_data['filter_data']);
		...
		$this->setCellValue($getActiveSheet, 'A1', 'DAW Registrations - 2024, DIAC');
