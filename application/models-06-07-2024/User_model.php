...
class User_model extends CI_model
{
	...
	public function login()
	{
		...
		$this->db->select('um.user_code, um.user_name, um.user_display_name, um.job_title, um.record_status, resom.resource_link, resom.is_maintenance, um.is_new_record, udm.user_logo, rm.role_name, rm.role_code, um.fk_state_code, um.diac_serial_no');
		...
		if ($userRecord->num_rows() == 1) {
			if ($this->session->userdata('captchaword') == $txt_captcha) {
				if ($userDataArr[0]['record_status'] != 0) {
					...
					$this->session->set_userdata('diac_serial_no', $userDataArr[0]['diac_serial_no']);
