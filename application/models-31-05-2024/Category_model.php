...
class Category_model extends CI_Model
{
	...
	public function get($data, $op)
	{
		switch ($op) {
			case 'GET_ALL_PANEL_CATEGORY':
				...
				$this->db->select('id,code, category_name,category_code');
	...
	public function post($data, $op)
	{
		switch ($op) {
			...
			// case 'GET_ALL_PANEL_CATEGORY_FOR_ARB_MASTER':
			// 	$this->db->select('pct.code,pct.category_name');
			// 	$this->db->from('panel_category_tbl as pct');
			// 	$this->db->where('status', 1);
			// 	$this->db->join('arbitrator_master_setup_tbl as amt', 'amt.category = pct.code', 'left');
