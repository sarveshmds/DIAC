...
if (!function_exists('get_email_data')) {
	function get_email_data($senderEmailId = DEFAULT_SMTP_EMAIL_ID)
	{
		$CI = &get_instance();
		$CI->load->database();

		$email_data = $CI->db->get_where('email_provider_setup', [
			'email_id' => $senderEmailId,
			'record_status' => 1
		])->row_array();

		if (!$email_data) {
			$default_email_data = $CI->db->get_where('email_provider_setup', [
				'email_id' => DEFAULT_SMTP_EMAIL_ID,
				'record_status' => 1
			])->row_array();
			...
			if ($default_email_data) {
				...
				return array('status' => false, 'msg' => "Error while setting the email provider setup details. Please check the SMTP settings.");
			}
		} else {
			return $email_data;
		}
 ...
 */
function sendEmails($to_email_id, $subject, $body, $smtp_email_id = DEFAULT_SMTP_EMAIL_ID, $cc_email_id = "", $bcc_email_id = "")
{

	try {

		$email_data = get_email_data($smtp_email_id);
		// $email_data = get_email_data('noreply@diac.ind.in');
		if (isset($email_data['status']) && $email_data['status'] == false) {
			return array('status' => $email_data['status'], 'msg' => $email_data['msg']);
		}
