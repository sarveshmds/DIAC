...
function automatic_case_allotment($case_no, $role)
{
    ...
    $case_ref_no_data = $CI->db->query("SELECT b.user_code, b.user_name, b.email, b.phone_number,b.user_display_name FROM master_case_ref_no_tbl a 
                LEFT JOIN user_master b ON a.code=b.`case_ref_no` AND b.record_status = 1
                WHERE `type`='" . $type . "' 
                AND start_index<= CAST(REPLACE(SUBSTRING('" . $case_no . "',-2),'00','100') AS UNSIGNED)
                AND REPLACE(end_index,'00','100')>=CAST(REPLACE(SUBSTRING('" . $case_no . "',-2),'00','100') AS UNSIGNED) AND b.record_status = 1")->row_array();
...
function getLinkedCaseDetails($case_slug)
{
    $CI = &get_instance();

    $CI->db->select("ctc.*, cdt2.case_no_prefix, cdt2.case_no, cdt2.case_no_year, cdt2.case_title as master_case_title, cdt2.case_no as tagged_case_desc, cdt2.case_title as tagged_case_title, um.user_display_name, um.job_title");
    $CI->db->from('cs_tagged_cases_tbl AS ctc');
    $CI->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ctc.master_case', 'left');
    $CI->db->join('cs_case_details_tbl as cdt2', 'cdt2.slug = ctc.tagged_case', 'left');
    $CI->db->join('user_master as um', 'um.user_code = ctc.created_by', 'left');
    $CI->db->where('ctc.record_status', 1);
    $CI->db->where("(ctc.master_case = '$case_slug' OR  ctc.tagged_case = '$case_slug')");

    $query = $CI->db->get();
    $fetch_data = $query->row_array();

    return $fetch_data;
}

function getLinkedMasterCase($case_slug)
{
    $CI = &get_instance();

    $CI->db->select("ctc.*, cdt.case_no_prefix, cdt.case_no, cdt.case_no_year, cdt.case_title as master_case_title, cdt2.case_no as tagged_case_desc, cdt2.case_title as tagged_case_title, um.user_display_name, um.job_title");
    $CI->db->from('cs_tagged_cases_tbl AS ctc');
    $CI->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ctc.master_case', 'left');
    $CI->db->join('cs_case_details_tbl as cdt2', 'cdt2.slug = ctc.tagged_case', 'left');
    $CI->db->join('user_master as um', 'um.user_code = ctc.created_by', 'left');
    $CI->db->where('ctc.record_status', 1);
    $CI->db->where("ctc.master_case = '$case_slug'");
...
function getAllLinkedWithMasterCase($case_slug)
{
    $CI = &get_instance();

    $CI->db->select("ctc.*, cdt.case_no_prefix, cdt.case_no, cdt.case_no_year, cdt.case_title as master_case_title, cdt2.case_no_prefix as tagged_case_no_prefix,  cdt2.case_no as tagged_case_desc, cdt2.case_no_year as tagged_case_no_year, cdt2.case_title as tagged_case_title, um.user_display_name, um.job_title");
    $CI->db->from('cs_tagged_cases_tbl AS ctc');
    $CI->db->join('cs_case_details_tbl as cdt', 'cdt.slug = ctc.master_case', 'left');
    $CI->db->join('cs_case_details_tbl as cdt2', 'cdt2.slug = ctc.tagged_case', 'left');
    $CI->db->join('user_master as um', 'um.user_code = ctc.created_by', 'left');
